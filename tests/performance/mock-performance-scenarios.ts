    ...
    summary: {
      ...
      criticalMetrics: {
        fcp: {
          ...
          improvement: ((baseline.metrics.FCP - optimized.metrics.FCP) / baseline.metrics.FCP * 100).toFixed(1)
        },
        tti: {
          baseline: baseline.metrics.TTI,
          optimized: optimized.metrics.TTI,
          improvement: ((baseline.metrics.TTI - optimized.metrics.TTI) / baseline.metrics.TTI * 100).toFixed(1)
        },
        bundleSize: {
          baseline: baseline.bundleAnalysis.totalSize,
          optimized: optimized.bundleAnalysis.totalSize,
          improvement: ((baseline.bundleAnalysis.totalSize - optimized.bundleAnalysis.totalSize) / baseline.bundleAnalysis.totalSize * 100).toFixed(1)
        }
      }
    },
    details: {
