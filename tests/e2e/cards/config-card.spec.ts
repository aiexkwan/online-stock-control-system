...
test.describe('ConfigCard E2E Tests', () => {
  ...
  test.describe('Basic Functionality', () => {
    ...
    test('should display configuration items with correct data types', async ({ page }) => {
      ...
      // Boolean type
      const booleanConfig = page.locator(
        '[data-testid="config-item"]:has-text("system.maintenance.mode")'
      );
      await expect(booleanConfig.locator('text=BOOLEAN')).toBeVisible();

      // Number type
      const numberConfig = page.locator(
        '[data-testid="config-item"]:has-text("system.api.rateLimit")'
      );
  ...
  test.describe('CRUD Operations', () => {
    ...
    test('should update existing configuration', async ({ page }) => {
      ...
      const editableConfig = page
        .locator('[data-testid="config-item"]')
        .filter({ hasText: 'system.api.rateLimit' })
        .first();
    ...
    test('should batch update multiple configurations', async ({ page }) => {
      ...
      if (count >= 2) {
        ...
        const editButtons = page.locator(
          '[data-testid="config-item"][data-selected="true"] button[aria-label="Edit configuration"]'
        );
  ...
  test.describe('Permissions', () => {
    ...
    test('should respect permission hierarchy', async ({ page }) => {
      ...
      const publicConfig = page
        .locator('[data-testid="config-item"]')
        .filter({ hasText: 'PUBLIC' });
      const authenticatedConfig = page
        .locator('[data-testid="config-item"]')
        .filter({ hasText: 'AUTHENTICATED' });
      const departmentConfig = page
        .locator('[data-testid="config-item"]')
        .filter({ hasText: 'DEPARTMENT' });
      const adminConfig = page.locator('[data-testid="config-item"]').filter({ hasText: 'ADMIN' });
      const superAdminConfig = page
        .locator('[data-testid="config-item"]')
      ...
      if ((await superAdminConfig.count()) > 0) {
        await expect(
          superAdminConfig.locator('[data-testid="high-security-indicator"]')
        ).toBeVisible();
      }
    });
  });
  ...
  test.describe('Real-time Updates', () => {
    ...
    test('should handle concurrent updates', async ({ page, context }) => {
      ...
      await expect(
        page2.locator('text=/Configuration has been modified|Conflict detected/')
      ).toBeVisible();
  ...
  test.describe('Accessibility', () => {
    test('should support keyboard navigation', async ({ page }) => {
      ...
      // Check tab changed
      await expect(page.locator('[role="tab"][aria-selected="true"]')).not.toHaveText(
        /SYSTEM_CONFIG/
      );
