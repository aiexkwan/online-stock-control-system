...
export function MenuBar({ items, activeItem, onItemClick, className }: MenuBarProps) {
  ...
  return (
    <div
      ref={menuRef}
      className={cn(
        'relative inline-flex items-center gap-1 rounded-full border border-slate-700/50 bg-slate-800/60 p-1 shadow-lg backdrop-blur-xl',
        className
      )}
    >
      ...
      {items.map(item => {
        ...
        return (
          <Link
            ...
            className={cn(
              'relative z-10 flex items-center gap-2 rounded-full px-2 py-2 text-sm font-medium transition-all duration-200 sm:px-4',
              isActive || isHovered ? 'text-white' : 'text-slate-400 hover:text-white'
            )}
          >
            {Icon && (
              <Icon
