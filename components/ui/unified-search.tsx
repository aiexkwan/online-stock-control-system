  ...
  (
    ...
    const handleSelect = useCallback(
      (result: SearchResult) => {
        ...
        // Strategy 4: unknown + type narrowing - 安全訪問 data 數組屬性
        const firstDataItem = result.data[0];
        const actualValue =
          firstDataItem &&
          typeof firstDataItem === 'object' &&
          'type' in firstDataItem &&
          firstDataItem.type === 'pallet'
            ? String((firstDataItem as Record<string, unknown>).value)
            : result.title;
      ...
      <div className='relative w-full'>
        ...
        <div className='relative'>
          <div className='pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3'>
            {isLoading ? (
              <div className='h-1 w-4 rounded-full bg-gray-400 opacity-75' />
            ) : (
