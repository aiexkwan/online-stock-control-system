...
function formatDataPreview(type: string, item: DatabaseRecord): string {
  switch (type) {
    ...
    case 'inventory_mismatch':
      return `${item.product_code}: System shows ${item.stock_level}, actual ${item.pallet_total} (${Math.round(typeof item.variance_percentage === 'number' ? item.variance_percentage : 0)}% variance)`;
