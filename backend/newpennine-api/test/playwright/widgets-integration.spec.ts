...
test.describe('v1.3.2 前端 Widgets 整合測試', () => {
  ...
  test('庫存分析圖表應該正常渲染', async ({ page }) => {
    ...
    // 等待庫存分析圖表載入
    await page.waitForSelector('[data-testid="inventory-analysis-chart"]', { timeout: 15000 });
    
    // 檢查圖表容器
    const chartContainer = page.locator('[data-testid="inventory-analysis-chart"]');
    await expect(chartContainer).toBeVisible();
    
    // 檢查是否有圖表數據
    const chartData = await chartContainer.locator('[data-testid="chart-data"]');
  ...
  test('產品分佈圓餅圖應該正常顯示', async ({ page }) => {
    ...
    // 等待產品分佈圖載入
    await page.waitForSelector('[data-testid="product-distribution-chart"]', { timeout: 15000 });
  ...
  test('訂單進度圖表應該響應日期篩選', async ({ page }) => {
    ...
    // 等待訂單進度圖表載入
    await page.waitForSelector('[data-testid="order-progress-chart"]', { timeout: 15000 });
    ...
    if (await dateFilter.isVisible()) {
      ...
      // 檢查圖表是否更新
      const chartContainer = page.locator('[data-testid="order-progress-chart"]');
      await expect(chartContainer).toBeVisible();
    }
  ...
  test('Widget 懶加載功能應該正常工作', async ({ page }) => {
    ...
    // 檢查 skeleton 載入狀態
    const skeletons = await page.locator('[data-testid="widget-skeleton"]').all();
    ...
    if (skeletons.length > 0) {
      ...
      // 檢查 skeleton 是否消失
      await expect(page.locator('[data-testid="widget-skeleton"]')).toHaveCount(0);
    }
  ...
  test('性能監控應該記錄 Web Vitals', async ({ page }) => {
    ...
    console.log(`✅ 性能指標正常 - DOMContentLoaded: ${timing.domContentLoaded}ms, LoadComplete: ${timing.loadComplete}ms`);
  });
});