...
export function LoginProvider({ 
  ...
  const register = useCallback(async (formData: RegisterFormData): Promise<LoginResult> => {
    try {
      ...
      if (!formData.email.endsWith('@pennineindustries.com')) {
        return { success: false, error: 'Only @pennineindustries.com email addresses are allowed' };
      }
  ...
  const setConfirmation = useCallback((show: boolean, message = '') => {
  ...
  const setCurrentView = useCallback((view: LoginUIState['currentView']) => {
    persistence.updateUIState({ currentView: view });
  }, [persistence]);
  
  // Form Actions
  const updateLoginForm = useCallback((field: keyof LoginFormData, value: string) => {
  ...
  const updateRegisterForm = useCallback((field: keyof RegisterFormData, value: string) => {
