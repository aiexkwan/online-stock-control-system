...
export function cleanupLegacyAuth() {
  ...
  isDev && console.log('[CleanupLegacyAuth] Starting cleanup of legacy authentication data');
  ...
  legacyKeys.forEach(key => {
    ...
    if (value) {
      isDev && console.log(`[CleanupLegacyAuth] Removing legacy localStorage key: ${key}`);
  ...
  legacyCookies.forEach(cookieName => {
    ...
    document.cookie = `${cookieName}=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/; domain=.${window.location.hostname};`;
    isDev && console.log(`[CleanupLegacyAuth] Cleared legacy cookie: ${cookieName}`);
  });
  ...
  document.cookie.split(';').forEach(cookie => {
    ...
    if (trimmedName.startsWith('sb-')) {
      ...
      isDev && console.log(`[CleanupLegacyAuth] Cleared Supabase cookie: ${trimmedName}`);
    }
  });
  ...
  for (let i = localStorage.length - 1; i >= 0; i--) {
    ...
    if (key?.startsWith('pennine_secure_')) {
      isDev && console.log(`[CleanupLegacyAuth] Found secure storage key: ${key}`);
      // 可選：如果需要完全重置，取消註釋下面這行
      // localStorage.removeItem(key);
    }
  }

  isDev && console.log('[CleanupLegacyAuth] Legacy cleanup completed');
}
...
export function forceCleanupAllAuth() {
  ...
  const isDev = process.env.NODE_ENV !== 'production';
  isDev && console.log('[CleanupLegacyAuth] Force cleanup all authentication data');
  ...
  for (let i = localStorage.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      localStorage.removeItem(key);
      isDev && console.log(`[CleanupLegacyAuth] Removed: ${key}`);
    }
  }
  ...
  isDev && console.log('[CleanupLegacyAuth] Force cleanup completed');
}
