...
export function useAuthEvents(options: UseAuthEventsOptions = {}) {
  ...
  const authEvents = {
    // Login events
    emitLoginAttempt: useCallback((email: string, password: string) => 
      emit('LOGIN_ATTEMPT', { email, password }), [emit]),
    
    emitLoginSuccess: useCallback((email: string, user: any, redirectPath?: string) => 
      emit('LOGIN_SUCCESS', { email, user, redirectPath }), [emit]),
    
    emitLoginError: useCallback((error: string, field?: string) => 
      emit('LOGIN_ERROR', { error, field }), [emit]),

    // Register events
    emitRegisterAttempt: useCallback((data: any) => 
      emit('REGISTER_ATTEMPT', data), [emit]),
    
    emitRegisterSuccess: useCallback((email: string) => 
      emit('REGISTER_SUCCESS', { email }), [emit]),
    
    emitRegisterError: useCallback((error: string, field?: string) => 
      emit('REGISTER_ERROR', { error, field }), [emit]),

    // Form events
    emitFormFieldChange: useCallback((field: string, value: string, formType: 'login' | 'register' | 'reset' | 'change') => 
      emit('FORM_FIELD_CHANGE', { field, value, formType }), [emit]),
    
    emitFormSubmit: useCallback((formType: 'login' | 'register' | 'reset' | 'change', data: any) => 
      emit('FORM_SUBMIT', { formType, data }), [emit]),

    // UI events
    emitViewChange: useCallback((from: any, to: any) => 
      emit('VIEW_CHANGE', { from, to }), [emit]),
    
    emitPasswordVisibilityToggle: useCallback((field: 'password' | 'confirmPassword', visible: boolean) => 
      emit('PASSWORD_VISIBILITY_TOGGLE', { field, visible }), [emit]),
    
    emitConfirmationShow: useCallback((message: string, type: 'success' | 'error' | 'info' = 'info') => 
      emit('CONFIRMATION_SHOW', { message, type }), [emit]),
    
    emitConfirmationHide: useCallback(() => 
      emit('CONFIRMATION_HIDE', {}), [emit]),

    // System events
    emitErrorClear: useCallback((scope?: 'field' | 'form' | 'all', field?: string) => 
      emit('ERROR_CLEAR', { scope, field }), [emit]),
    
    emitStateReset: useCallback((scope: 'forms' | 'ui' | 'errors' | 'all') => 
      emit('STATE_RESET', { scope }), [emit]),
    
    emitLoadingState: useCallback((loading: boolean, operation?: string) => 
      emit('LOADING_STATE', { loading, operation }), [emit]),
  };

  // Event history access
  const getHistory = useCallback((eventType?: keyof EventMap, limit?: number) => {
