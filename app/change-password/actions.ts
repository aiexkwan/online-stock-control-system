...
export async function updateUserPasswordInDbAction(
  ...
  if (userError) {
    console.error('[updateUserPasswordInDbAction] Error fetching user from session:', userError.message);
  ...
  if (!clockNumber) {
    ...
    console.error('[updateUserPasswordInDbAction] Clock number not found in user metadata for user ID:', user.id);
  ...
  if (result.error) {
    console.error(`[updateUserPasswordInDbAction] Error from updatePasswordWithSupabaseAuth: ${result.error}`);
