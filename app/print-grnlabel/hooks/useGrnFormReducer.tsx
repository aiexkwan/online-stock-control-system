...
export type GrnFormAction =
  ...
  | { type: 'UPDATE_PROGRESS_STATUS'; index: number; status: GrnFormState['progress']['status'][number] }
  | { type: 'RESET_FORM' }
...
export const useGrnFormReducer = () => {
  ...
  // 輔助函數 - 更新進度狀態
  const updateProgressStatus = useCallback((index: number, status: GrnFormState['progress']['status'][number]) => {
  ...
  const actions = useMemo(() => ({
    ...
    setProductInfo: (productInfo: GrnFormState['productInfo']) => dispatch({ type: 'SET_PRODUCT_INFO', productInfo }),
    setSupplierInfo: (supplierInfo: GrnFormState['supplierInfo']) => dispatch({ type: 'SET_SUPPLIER_INFO', supplierInfo }),
    ...
    setProgress: (progress: GrnFormState['progress']) => dispatch({ type: 'SET_PROGRESS', progress }),
