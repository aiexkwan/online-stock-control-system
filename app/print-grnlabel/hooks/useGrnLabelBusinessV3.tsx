  ...
  const processPrintRequest = useCallback(
    async (clockNumber: string) => {
      ...
      try {
        ...
        const result = await palletGeneration.generatePalletNumbers(
        ...
        try {
          ...
          if (batchResult.success) {
            ...
            if (
              rpcPalletNumbers.length === numberOfPalletsToProcess &&
              rpcSeries.length === numberOfPalletsToProcess
            ) {
              // 更新本地變數
          ...
          if (!rollbackResult.success) {
            ...
            if (palletNumbers.length > 0 && series.length > 0) {
              try {
                ...
                process.env.NODE_ENV !== 'production' &&
                  console.log('[useGrnLabelBusinessV3] Manual pallet rollback successful');
              } catch (manualRollbackError) {
                console.error(
                  '[useGrnLabelBusinessV3] Manual pallet rollback failed:',
                  manualRollbackError
                );
              }
            }
    ...
    [state, actions, weightCalculation, palletGeneration, currentUserId, printGrnLabels]
  );
