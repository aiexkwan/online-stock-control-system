...
export function useBatchVoid() {
  ...
  const addToBatch = useCallback(
    async (searchValue: string, searchType: 'qr' | 'pallet_num') => {
      ...
      const isDuplicate = batchState.items.some(
        item => item.palletInfo.plt_num === searchValue || item.palletInfo.series === searchValue
      );
  ...
  const toggleItemSelection = useCallback((id: string, selected: boolean) => {
    setBatchState(prev => {
      const items = prev.items.map(item => (item.id === id ? { ...item, selected } : item));
  ...
  // Execute batch void
  const executeBatchVoid = useCallback(
    async (params: { voidReason: string; password: string; damageQuantity?: number }) => {
      ...
      if (successCount > 0 && errorCount === 0) {
        ...
        toast.warning(
          `Voided ${successCount} pallet${successCount > 1 ? 's' : ''}, ${errorCount} failed`
        );
      } else {
