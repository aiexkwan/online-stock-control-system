...
export function useVoidPallet() {
  ...
  const shouldShowReprintDialog = useCallback(
    (voidReason: string, result: unknown, palletInfo: PalletInfo): boolean => {
      // 🔥 修改：檢查是否為 ACO pallet
      ...
      // 🔥 修改：完全損壞不顯示重印對話框
      // 策略4: unknown + type narrowing - 安全獲取 remainingQty 屬性
      const remainingQty =
        result && typeof result === 'object' && 'remainingQty' in result
          ? (result as { remainingQty: unknown }).remainingQty
          : undefined;
      if (voidReason === 'Damage' && typeof remainingQty === 'number' && remainingQty === 0) {
        return false;
      }

      // 只有在選擇了特定的作廢原因後才顯示重印對話框
      const reprintReasons = ['Wrong Label', 'Wrong Qty', 'Wrong Product Code', 'Damage'];
      // 策略4: unknown + type narrowing - 安全獲取 success 屬性
      const success =
        result && typeof result === 'object' && 'success' in result
          ? Boolean((result as { success: unknown }).success)
          : false;
      return reprintReasons.includes(voidReason) && success;
    },
    []
  );
