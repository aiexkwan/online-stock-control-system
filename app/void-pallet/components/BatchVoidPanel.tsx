      ...
      <div className="max-h-96 overflow-y-auto custom-scrollbar">
        {items.length === 0 ? (
          <div className="p-8 text-center text-gray-400">
            ...
            <p className="text-sm mt-1">Scan pallets to add them to the batch</p>
          </div>
        ) : (
          <div className="p-2">
            <AnimatePresence>
              {items.map((item, index) => (
                <motion.div
                  ...
                  className={`
                    mb-2 p-3 rounded-lg border transition-all duration-200
                    ${getStatusColor(item.status)}
                    ${item.selected && item.status === 'pending' ? 'ring-2 ring-orange-500' : ''}
                  `}
                >
