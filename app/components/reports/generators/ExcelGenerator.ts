...
export class ExcelGenerator implements ReportGenerator {
  ...
  async generate(data: ProcessedReportData, config: ReportConfig): Promise<Blob> {
    ...
    if (data.summary && Object.keys(data.summary).length > 0) {
      this.addSummarySheet(workbook, data.summary, config);
    }
    ...
    for (const section of config.sections) {
      ...
      if (section.type === 'table' && Array.isArray(sectionData)) {
        this.addDataSheet(workbook, section, sectionData, config);
      }
    }
    
    // 添加元數據工作表
    this.addMetadataSheet(workbook, data.metadata, config);
  ...
  private applyStyles(worksheet: XLSX.WorkSheet, config: ReportConfig): void {
    // 注意：xlsx 庫的社區版不支援樣式
