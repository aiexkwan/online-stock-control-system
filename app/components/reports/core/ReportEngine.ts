...
export class ReportEngine {
  ...
  private async fetchData(filters: FilterValues): Promise<Record<string, any>> {
    ...
    const fetchPromises = this.config.sections.map(async (section) => {
      ...
      if (!dataSource) {
        throw new Error(`Data source "${section.dataSource}" not found for section "${section.id}"`);
      }
          ...
          switch (field.type) {
            ...
            case 'sum':
              if (field.field && Array.isArray(sectionData)) {
                summary[field.id] = sectionData.reduce((sum, item) => 
                  sum + (Number(item[field.field as keyof typeof item]) || 0), 0
                );
              }
              break;
            case 'average':
              if (field.field && Array.isArray(sectionData) && sectionData.length > 0) {
                const sum = sectionData.reduce((sum, item) => 
                  sum + (Number(item[field.field as keyof typeof item]) || 0), 0
                );
