/**
 * @deprecated This hook is deprecated. Please use Server Actions from '@/app/actions/grnActions' directly instead.
 * This hook uses client-side Supabase operations which goes against our unified Server Actions architecture.
 *
 * @example
 * ```typescript
 * // OLD (deprecated)
 * const { generatePalletNumbersAndSeries } = useDatabaseOperationsV2();
 *
 * // NEW (recommended)
 * import { createGrnDatabaseEntries } from '@/app/actions/grnActions';
 * const result = await createGrnDatabaseEntries(payload, operatorClock, labelMode);
 * ```
 */
...
export function useDatabaseOperationsV2() {
  ...
  const generatePalletNumbersAndSeries = useCallback(
    async (count: number): Promise<GenerationResult> => {
      try {
        process.env.NODE_ENV !== 'production' &&
          process.env.NODE_ENV !== 'production' &&
          console.log(
            `[GRN useDatabaseOperationsV2] Generating ${count} pallet numbers using optimized method`
          );
  ...
  const createDatabaseEntries = useCallback(
    async (
  ...
  const uploadPdf = useCallback(
    async (pdfBlob: Blob, fileName: string, storagePath: string = 'grn-labels') => {
      try {
