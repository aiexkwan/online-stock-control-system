...
import { type GrnPalletInfoPayload, type GrnRecordPayload } from '@/app/actions/grnActions';
import {
  confirmPalletUsage,
...
export function useGrnLabelBusiness() {
  ...
  const { generatePalletNumbersAndSeries, createDatabaseEntries, uploadPdf } =
    useDatabaseOperationsV2();
  const { generateMultiplePdfs, generateSinglePdf, printPdfs, generationProgress, resetProgress } =
    usePdfGeneration();
  const { saveFormData, clearFormData } = useFormPersistence();

  // Process GRN labels
  const processGrnLabels = useCallback(
    async (
      formData: FormData,
      ...
      try {
        ...
        const selectedPalletType =
          Object.entries(formData.palletType).find(
            ([, value]) => (parseInt(value) || 0) > 0
          )?.[0] || 'notIncluded';
        const selectedPackageType =
          Object.entries(formData.packageType).find(
            ([, value]) => (parseInt(value) || 0) > 0
          )?.[0] || 'notIncluded';

        const palletCount = parseInt(
          formData.palletType[selectedPalletType as keyof typeof formData.palletType] || '0'
        );
        const packageCount = parseInt(
          formData.packageType[selectedPackageType as keyof typeof formData.packageType] || '0'
        );
          ...
          await Promise.all(
            batch.map(async (data, batchIndex) => {
              ...
              try {
                ...
                console.error(
                  `[GRN useGrnLabelBusiness] Error processing pallet ${index + 1}:`,
                  error
                );
