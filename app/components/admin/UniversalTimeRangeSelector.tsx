...
export const UniversalTimeRangeSelector: React.FC<UniversalTimeRangeSelectorProps> = ({
  ...
  const handleApply = () => {
    if (tempDateRange?.from) {
      ...
      const label = format(tempDateRange.from, 'yyyy-MM-dd') === format(endDate, 'yyyy-MM-dd')
      ...
      {isOpen && (
        <>
          ...
          <div className={cn(
            ...
            {/* Action buttons */}
            <div className="mt-4 flex gap-2 justify-end border-t border-slate-700/50 pt-4">
              <button
                onClick={handleCancel}
                className="px-4 py-2 text-sm bg-slate-800/50 hover:bg-slate-700/50 rounded-lg transition-colors text-gray-300 hover:text-white"
              >
                Cancel
              </button>
              <button
