...
export const ResponsiveGrid: React.FC<ResponsiveGridProps> = React.memo(
  ({ children, columns = { sm: 1, md: 2, lg: 2, xl: 3 }, gap = 6, className = '' }) => {
...
export const ResponsiveContainer: React.FC<ResponsiveContainerProps> = React.memo(
  ({ children, maxWidth = 'lg', padding = true, className = '' }) => {
...
export const ResponsiveCard: React.FC<{
  ...
  ({ children, title, subtitle, className = '', padding = 'md', shadow = true, headerAction }) => {
    ...
    return (
      <div
        className={`group relative overflow-hidden rounded-2xl border border-slate-600/30 bg-gradient-to-br from-slate-800/60 via-slate-800/40 to-blue-900/30 backdrop-blur-xl ${shadow ? 'shadow-2xl shadow-blue-900/10 transition-all duration-300 hover:shadow-blue-800/20' : ''} ${paddingClasses[padding]} ${className} hover:border-blue-500/30 hover:bg-gradient-to-br hover:from-slate-800/70 hover:via-slate-800/50 hover:to-blue-900/40`}
      >
        {/* 卡片內部光效 */}
        <div className='absolute inset-0 bg-gradient-to-r from-blue-500/5 via-transparent to-cyan-500/5 opacity-0 transition-opacity duration-500 group-hover:opacity-100'></div>

        {/* 頂部邊框光效 */}
        <div className='absolute left-0 right-0 top-0 h-px bg-gradient-to-r from-transparent via-blue-400/50 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100'></div>
...
export const ResponsiveStack: React.FC<{
  ...
  ({ children, direction = 'vertical', spacing = 4, align = 'stretch', className = '' }) => {
