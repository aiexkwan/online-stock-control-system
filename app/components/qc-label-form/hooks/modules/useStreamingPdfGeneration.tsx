...
export const useStreamingPdfGeneration = (): UseStreamingPdfGenerationReturn => {
  ...
  const generateSinglePdf = useCallback(async (
    ...
    try {
      ...
      if (updateResult.error) {
        console.error(`[useStreamingPdfGeneration] Failed to update PDF URL for pallet ${palletNum}:`, updateResult.error);
  ...
  // 批量串流生成 PDFs
  const generatePdfsStream = useCallback(async (options: StreamingPdfGenerationOptions) => {
    const {
      productInfo,
      quantity,
      count,
