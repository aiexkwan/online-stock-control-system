    ...
    (error: unknown): { type: ErrorDetails['type']; severity: ErrorDetails['severity'] } => {
      ...
      if (message.includes('database') || message.includes('sql') || message.includes('supabase')) {
        return {
          type: 'database',
          severity:
            message.includes('connection') || message.includes('timeout') ? 'high' : 'medium',
        };
      }
    ...
    async (error: unknown, context?: Record<string, unknown>): Promise<ErrorDetails> => {
      ...
      if (finalConfig.toastNotifications) {
        if (severity === 'critical' || severity === 'high') {
          toast.error(
            `Critical Error: ${errorDetails.message.substring(0, 100)}${errorDetails.message.length > 100 ? '...' : ''}`
          );
        } else if (severity === 'medium') {
          toast.warning(
            `Warning: ${errorDetails.message.substring(0, 80)}${errorDetails.message.length > 80 ? '...' : ''}`
          );
        }
      }
  ...
  const retryError = useCallback(
    async (errorId: string): Promise<boolean> => {
      ...
      try {
        // 更新重試計數
        setErrors(prev =>
          prev.map(e => (e.id === errorId ? { ...e, retryCount: e.retryCount + 1 } : e))
        );
    ...
    if (startTime) {
      ...
      setErrorStats(prev => {
        ...
        const totalRecoveryTime =
          newStats.averageRecoveryTime * (newStats.recoveredErrors - 1) + recoveryTime;
