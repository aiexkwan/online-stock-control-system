 ...
 *
 * @deprecated This hook is deprecated. Please use Server Actions from '@/app/actions/qcActions' instead.
 * This hook uses client-side Supabase operations which goes against our unified Server Actions architecture.
 *
 * @example
 * ```typescript
 * // OLD (deprecated)
 * const { processQcLabelUnified } = useDatabaseOperationsUnified();
 *
 * // NEW (recommended)
 * import { createQcDatabaseEntries } from '@/app/actions/qcActions';
 * const result = await createQcDatabaseEntries(options);
 * ```
 */
...
export const useDatabaseOperationsUnified = (): UseDatabaseOperationsUnifiedReturn => {
  ...
  // 統一處理 QC 標籤的所有操作
  const processQcLabelsUnified = useCallback(
    async (options: UnifiedQcOptions): Promise<UnifiedDatabaseResult> => {
      const { productInfo, quantity, count, clockNumber, formData, pdfUrls, sessionId } = options;
      ...
      try {
        ...
        const acoQuantityUsed = productInfo.type === 'ACO' && acoOrderRef ? quantity * count : null;

        const slateBatchNumber =
          productInfo.type === 'Slate' && formData.slateDetail?.batchNumber?.trim()
