...
export default async function AdminThemePage({ params }: AdminThemePageProps) {
  ...
  try {
    ...
    if (criticalThemes.includes(finalTheme)) {
      ...
      console.log(
        `[SSR as string] Successfully prefetched ${Object.keys(prefetchedData).length} critical widgets`
      );
    }
  } catch (error) {
...
export async function generateStaticParams() {
  return [{ theme: 'operations-monitoring' }, { theme: 'data-management' }, { theme: 'analytics' }];
}
