...
export default function BusinessMetricsCard({
  ...
  const successRate =
    data.performance.apiRequests.total > 0
      ? (data.performance.apiRequests.successful / data.performance.apiRequests.total) * 100
      : 0;
      ...
      <Card className='w-full'>
        <CardHeader className='pb-2'>
          <CardTitle className='flex items-center justify-between text-base'>
            ...
            <Badge
              variant={(data as { status: string }).status === 'healthy' ? 'default' : 'secondary'}
            >
  ...
  return (
    <Card className='w-full'>
      <CardHeader>
        ...
        <CardDescription>Key business indicators and performance metrics</CardDescription>
      </CardHeader>
      <CardContent className='space-y-6'>
        {/* 關鍵 KPI */}
        <div>
          <h3 className='mb-3 text-lg font-medium'>Key Performance Indicators</h3>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4'>
            <div className='rounded-lg border p-4'>
              <div className='mb-2 flex items-center justify-between'>
                <Users className='h-5 w-5 text-blue-500' />
                <div
                  className={`flex items-center space-x-1 ${getChangeColor(data.kpis.activeUsers.changeType)}`}
                >
            ...
            <div className='rounded-lg border p-4'>
              <div className='mb-2 flex items-center justify-between'>
                ...
                <div
                  className={`flex items-center space-x-1 ${getChangeColor(data.kpis.totalOrders.changeType)}`}
                >
            ...
            <div className='rounded-lg border p-4'>
              <div className='mb-2 flex items-center justify-between'>
                ...
                <div
                  className={`flex items-center space-x-1 ${getChangeColor(data.kpis.totalProducts.changeType)}`}
                >
            ...
            <div className='rounded-lg border p-4'>
              <div className='mb-2 flex items-center justify-between'>
                ...
                <span className='text-sm text-gray-500'>
                  Target: {data.kpis.responseTime.target}ms
                </span>
              </div>
        ...
        {/* 系統性能指標 */}
        <div>
          <h3 className='mb-3 text-lg font-medium'>System Performance</h3>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-3'>
            <div className='space-y-2'>
              <div className='flex items-center justify-between'>
                <span className='text-sm font-medium'>System Uptime</span>
                <span className='text-sm'>
                  {formatPercentage(data.kpis.systemUptime.value, data.kpis.systemUptime.target)}%
                </span>
              </div>
              <Progress
                value={parseFloat(
              ...
              <p className='text-xs text-gray-500'>
                Target: {data.kpis.systemUptime.target}% uptime
              </p>
            </div>
            ...
            <div className='space-y-2'>
              ...
              <Progress value={successRate} className='h-2' />
              <p className='text-xs text-gray-500'>
                {data.performance.apiRequests.successful} / {data.performance.apiRequests.total}{' '}
                requests
              </p>
            </div>
        ...
        {/* 用戶活動 */}
        <div>
          <h3 className='mb-3 text-lg font-medium'>User Activity</h3>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-3'>
            <div className='rounded-lg border p-4 text-center'>
              <Activity className='mx-auto mb-2 h-8 w-8 text-blue-500' />
              <p className='text-2xl font-bold'>
                {formatNumber(data.performance.userActivity.dailyActiveUsers)}
              </p>
            ...
            <div className='rounded-lg border p-4 text-center'>
              ...
              <p className='text-2xl font-bold'>
                {formatNumber(data.performance.userActivity.weeklyActiveUsers)}
              </p>
            ...
            <div className='rounded-lg border p-4 text-center'>
              ...
              <p className='text-2xl font-bold'>
                {formatNumber(data.performance.userActivity.monthlyActiveUsers)}
              </p>
        ...
        {/* API 請求統計 */}
        <div>
          <h3 className='mb-3 text-lg font-medium'>API Request Statistics</h3>
          <div className='grid grid-cols-1 gap-4 md:grid-cols-4'>
            <div className='text-center'>
              <p className='text-2xl font-bold text-blue-600'>
                {formatNumber(data.performance.apiRequests.total)}
              </p>
              <p className='text-sm text-gray-500'>Total Requests</p>
            </div>
            <div className='text-center'>
              <p className='text-2xl font-bold text-green-600'>
                {formatNumber(data.performance.apiRequests.successful)}
              </p>
              <p className='text-sm text-gray-500'>Successful</p>
            </div>
            <div className='text-center'>
              <p className='text-2xl font-bold text-red-600'>
                {formatNumber(data.performance.apiRequests.failed)}
              </p>
              <p className='text-sm text-gray-500'>Failed</p>
            </div>
            <div className='text-center'>
              <p className='text-2xl font-bold text-orange-600'>
                {data.performance.apiRequests.averageResponseTime}ms
              </p>
