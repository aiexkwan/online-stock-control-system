 ...
 * - 無障礙設計支援
 */
export default function SystemHealthCard({ data, compact = false, onRefresh }: SystemHealthCardProps) {
  if (!data) {
    return (
  ...
  const memoryUsagePercentage = data.systemMetrics ? 
    (data.systemMetrics.memoryUsage.heapUsed / data.systemMetrics.memoryUsage.heapTotal) * 100 : 0;
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium">
                      {formatBytes(data.systemMetrics.memoryUsage.heapUsed)} / {formatBytes(data.systemMetrics.memoryUsage.heapTotal)}
                    </span>
                  </div>
                  <Progress value={memoryUsagePercentage} className="h-2" />
                ...
                <div className="space-y-2">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <p className="text-gray-600">User: {(data.systemMetrics.cpuUsage.user / 1000).toFixed(1)}ms</p>
                    </div>
                    <div>
                      <p className="text-gray-600">System: {(data.systemMetrics.cpuUsage.system / 1000).toFixed(1)}ms</p>
                    </div>
                  </div>
                </div>
