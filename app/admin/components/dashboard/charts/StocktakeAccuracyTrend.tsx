...
export default function StocktakeAccuracyTrend({ timeFrame }: StocktakeAccuracyTrendProps) {
  ...
  const chartData = useMemo(() => {
    ...
    return Array.from(dateMap.entries())
      .map(([date, stats]) => {
        const accuracy = stats.expected > 0 
          ? Math.round((stats.counted / stats.expected) * 100 * 10) / 10 
          : 100;
  ...
  if (error) {
    return (
      <Alert variant="destructive">
        ...
        <AlertDescription>
          Failed to load stocktake data: {error.message}
        </AlertDescription>
      </Alert>
    );
  }
  ...
  const oldAvg = oldData.length > 0 ? oldData.reduce((sum, d) => sum + d.accuracy, 0) / oldData.length : recentAvg;
    ...
    <div className="w-full h-full flex flex-col">
      <div className="mb-4">
        <p className="text-sm text-white/60">
          Stocktake accuracy trend over the last 30 days
        </p>
      ...
      <div className="mt-4 grid grid-cols-3 gap-4 text-xs border-t pt-4">
        <div className="text-center">
          <div className="text-2xl font-bold text-green-600">{chartData.filter(d => d.accuracy >= 95).length}</div>
          <div className="text-white/60">Days Meeting Target (≥95%)</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-amber-600">{chartData.filter(d => d.accuracy >= 90 && d.accuracy < 95).length}</div>
          <div className="text-white/60">Near Target (90-94%)</div>
        </div>
        <div className="text-center">
          <div className="text-2xl font-bold text-red-600">{chartData.filter(d => d.accuracy < 90).length}</div>
