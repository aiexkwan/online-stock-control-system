...
import { DocumentArrowUpIcon, ArrowPathIcon, WifiIcon, ArrowsRightLeftIcon } from '@heroicons/react/24/outline';
...
export const OrdersListWidgetV2 = React.memo(function OrdersListWidgetV2({ 
  ...
  const handleOrderClick = useCallback(async (orderRef: string) => {
    ...
    try {
      ...
      console.error('[OrdersListWidgetV2] Error opening PDF:', error);
  ...
  const ConnectionStatus = () => (
    <div className="flex items-center gap-1.5 text-xs">
      {isRealtimeConnected ? (
        <>
          <WifiIcon className="w-3 h-3 text-emerald-400" />
          <span className="text-emerald-400">Real-time</span>
        </>
      ) : isPolling ? (
        <>
          <ArrowsRightLeftIcon className="w-3 h-3 text-amber-400 animate-pulse" />
        ...
        {error && !loading && (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-red-400 text-sm mb-2">Error loading orders</p>
              <button
                onClick={refresh}
            ...
            <AnimatePresence mode="popLayout">
              {orders.map((order, index) => (
                <motion.div 
                  ...
                  className={cn(
                    "bg-black/20 rounded-lg p-2 transition-colors cursor-pointer",
                    "hover:bg-white/10",
                ...
                >
                  <div className="grid grid-cols-3 gap-2 items-center">
                    ...
                    <button
                      ...
                      disabled={loadingPdf === order.remark || order.uploader_name === 'Loading...'}
                      className={cn(
                        "text-xs text-cyan-400 text-center truncate",
