...
export const ReportsWidget = React.memo(function ReportsWidget({
  widget,
  isEditMode,
      ...
      <WidgetCard widgetType='REPORTS' isEditMode={isEditMode}>
        <CardHeader className='pb-3'>
          <CardTitle className='flex items-center gap-2'>
            <DocumentChartBarIcon className='h-6 w-6 text-emerald-400' />
          ...
          <div className='grid grid-cols-2 gap-3'>
            {reportTypes.map(report => (
              <Button
                key={report.id}
                variant='ghost'
                className={`${report.color} flex h-32 flex-col items-center justify-center gap-3 p-4 text-white transition-all duration-300`}
