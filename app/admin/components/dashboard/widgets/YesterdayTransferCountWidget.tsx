  ...
  useEffect(() => {
    const fetchTransferCount = async () => {
      ...
      try {
        ...
        // Extract widget data
        const widgetData = dashboardResult.widgets?.find(w => w.widgetId === 'transfer_count');
    ...
    <WidgetCard widgetType={widget.type.toUpperCase() as keyof typeof WidgetStyles.borders}>
      <CardHeader className='pb-2'>
        ...
        <p className='mt-1 text-xs text-slate-400'>
          From {format(new Date(data.dateRange.start || dateRange.start), 'MMM d')} to{' '}
          {format(new Date(data.dateRange.end || dateRange.end), 'MMM d')}
        </p>
      </CardHeader>
