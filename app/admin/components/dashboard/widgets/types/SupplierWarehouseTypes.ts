...
export function isValidSupplierData(data: unknown): data is SupplierData {
  ...
  return (
    typeof s.supplier_code === 'string' &&
    typeof s.supplier_name === 'string'
  );
}
...
export class SupplierWarehouseMapper {
  static transformSupplierSearchResponse(response: unknown): SupplierData[] {
    ...
    return collection.edges
      .filter((edge): edge is { node: Record<string, unknown> } => 
  ...
  static calculateTransferStats(transfers: WarehouseTransferData[]): TransferStats {
    ...
    const completedTransfers = transfers.filter(t => 
      t.status === 'completed' && t.completed_date && t.transfer_date
    );
    ...
    const averageTime = completedTransfers.length > 0 
      ? completedTransfers.reduce((sum, t) => {
        ...
        }, 0) / completedTransfers.length / (1000 * 60 * 60) // Convert to hours
      : 0;
  ...
  static calculateWorkLevelStats(workLevels: WarehouseWorkLevelData[]): WorkLevelStats {
    ...
    // Find peak hour
    const hourlyTotals = workLevels.reduce((acc, wl) => {
    ...
    const warehouseTotals = workLevels.reduce((acc, wl) => {
      acc[wl.warehouse] = (acc[wl.warehouse] || 0) + wl.work_level;
      return acc;
    }, {} as Record<string, number>);

    const busiestWarehouse = Object.entries(warehouseTotals)
      .sort(([,a], [,b]) => b - a)[0]?.[0] || '';
...
export class SupplierValidator {
  static validateSupplierCode(code: string): FormValidationResult {
    ...
    if (!code.trim()) {
      ...
      errors.supplier_code = 'Supplier code can only contain letters, numbers, underscore and hyphen';
    }
