  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const widgetData = result.widgets?.find(w => w.widgetId === 'top_products_distribution');
        ...
        if (
          ...
          const chartData: ChartData[] = dataArray.map(
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className='rounded-lg border border-slate-700 bg-slate-800 p-2 shadow-lg'>
          ...
          <p className='text-xs text-slate-400'>Quantity: {data.value.toLocaleString()}</p>
