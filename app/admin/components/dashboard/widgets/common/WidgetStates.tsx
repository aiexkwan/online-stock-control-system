      ...
      <div className={cn('space-y-3', className)}>
        ...
        <div className='flex items-end justify-between gap-2' style={{ height }}>
          {Array.from({ length: 6 }).map((_, index) => (
            <div
              key={index}
              className='flex-1 animate-pulse rounded-t bg-slate-700'
              style={{
  ...
  if (type === 'chart-area') {
    return (
      <div className={cn('space-y-3', className)}>
        ...
        <div className='w-full animate-pulse rounded bg-slate-700' style={{ height }} />
      </div>
    );
  }
  ...
  if (type === 'list') {
    return (
      <div className={cn('space-y-3', className)}>
        ...
        {Array.from({ length: rows }).map((_, index) => (
          <div key={index} className='flex items-center gap-3 rounded-lg bg-slate-800/30 p-3'>
...
export const WidgetError = React.memo(function WidgetError({
  ...
  const defaultIcon =
    severity === 'info' ? <AlertCircle className='h-8 w-8' /> : <AlertCircle className='h-8 w-8' />;
  ...
  if (display === 'compact') {
    return (
      <div className={cn('flex items-center gap-2 p-2', className)}>
        {!hideIcon && (
          <div className={cn(styles.text)}>{icon || <AlertCircle className='h-4 w-4' />}</div>
        )}
        ...
        <div className='flex items-start gap-3'>
          ...
          <div className='flex-1'>
            <div className='flex items-center gap-2'>
              <h4 className={cn('text-sm font-medium', styles.text)}>{message}</h4>
    ...
    >
      {!hideIcon && (
        <div className={cn('mb-4 rounded-full p-3', styles.bg)}>
          <div className={styles.text}>{icon || defaultIcon}</div>
        </div>
      )}

      <h3 className={cn('mb-2 text-base font-semibold', styles.text)}>{message}</h3>

      {code && <p className='mb-2 text-sm text-gray-500'>Error Code: {code}</p>}
...
export const WidgetEmpty = React.memo(function WidgetEmpty({
  ...
  return (
    ...
    >
      ...
      <h3 className='mb-2 text-base font-medium text-slate-300'>{message}</h3>
...
export const WidgetStateWrapper = React.memo(function WidgetStateWrapper({
  ...
  if (error) {
    const message =
      errorMessage || (typeof error === 'string' ? error : (error as { message: string }).message);
    return (
      <div className={className}>
        <WidgetError
          message={message}
...
export const WidgetSuspenseFallback = React.memo(function WidgetSuspenseFallback({
    ...
    return (
      <div className={`${baseClasses} ${className || ''}`}>
        <div className='w-full space-y-3 p-4'>
          <div className='h-4 w-24 rounded bg-slate-700/60'></div>
          <div className='h-8 w-32 rounded bg-slate-700/60'></div>
          <div className='h-3 w-20 rounded bg-slate-700/40'></div>
        </div>
      </div>
    );
  }
  ...
  if (type === 'chart') {
    return (
      <div className={`${baseClasses} ${className || ''}`}>
        <div className='w-full space-y-3 p-4'>
          <div className='mb-4 h-4 w-32 rounded bg-slate-700/60'></div>
          <div className='h-32 w-full rounded bg-slate-700/40'></div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`${baseClasses} ${className || ''}`}>
        <div className='w-full space-y-2 p-4'>
          <div className='mb-3 h-4 w-32 rounded bg-slate-700/60'></div>
          {[1, 2, 3].map((i: number) => (
            <div key={i} className='h-6 w-full rounded bg-slate-700/40'></div>
          ))}
        </div>
      </div>
    );
        ...
        <div className='w-full space-y-3 p-4'>
          ...
          {[1, 2, 3].map((i: number) => (
            <div key={i} className='flex items-center gap-3'>
              <div className='h-8 w-8 rounded bg-slate-700/60'></div>
              <div className='flex-1 space-y-1'>
