...
interface BusinessMetricsData {
  ...
  kpis: {
    ...
    totalProducts: { value: number; change: number; changeType: 'increase' | 'decrease' | 'stable' };
...
export function useMonitoringData() {
  ...
  const [databasePerformance, setDatabasePerformance] = useState<DatabasePerformanceData | null>(null);
      ...
      const businessData: BusinessMetricsData = {
        ...
        performance: {
          apiRequests: {
            ...
            successful: (data.apiVersions?.totalRequests || 0) - (data.apiVersions?.totalErrors || 0),
