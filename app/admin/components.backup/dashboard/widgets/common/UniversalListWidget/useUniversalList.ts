/**
 * Universal List Hook
 * 統一的列表數據獲取 hook，整合現有的 useUnifiedAPI 和 useInViewport
 */
...
export function useUniversalList<T = any>(
  ...
  const {
    ...
    restEndpoint: config.dataSource.endpoint || '/api/dashboard/widgets/list',
    variables,
    ...
    widgetId: `list-${config.display.title}`,
  });
  ...
  const processedData = useMemo(() => {
    ...
    return result;
  }, [rawData, config.dataSource, config.plugins, filters, sort, timeFrame]);
  ...
  const performanceMetrics: PerformanceMetrics = useMemo(() => {
    ...
    return metrics;
  }, [queryMetrics, config.display.title]);
