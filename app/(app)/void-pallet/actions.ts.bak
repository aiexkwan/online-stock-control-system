...
function isACOOrderPallet(plt_remark: string | null): { isACO: boolean; refNumber?: string } {
  if (!plt_remark) {
    process.env.NODE_ENV !== 'production' && console.log('[ACO Check] No plt_remark found');
    return { isACO: false };
  }
  ...
  process.env.NODE_ENV !== 'production' && console.log('[ACO Check] No ACO reference found');
  return { isACO: false };
}
...
async function updateACORecord(
  ...
  try {
    ...
    if (!acoRecord) {
      process.env.NODE_ENV !== 'production' && console.log(`[ACO Update] No ACO record returned`);
      return {
        success: false,
        error: `ACO record not found for ref: ${refNumber}, code: ${productCode}`,
      };
...
export async function searchPalletAction(params: SearchParams): Promise<SearchResult> {
  ...
  try {
    ...
    if (!searchResult.pallet) {
      ...
      const alternateResult = await inventoryService.searchPallet(
        alternateType,
        searchValue.trim()
      );
    ...
    return {
      success: false,
      error: `Search failed: ${getErrorMessage(error) || 'Unknown error'}`,
    };
  }
}
...
export async function recordHistoryAction(
  ...
  try {
    ...
    return {
      success: false,
      error: `Failed to record history: ${getErrorMessage(error)}`,
    };
  }
}
 ...
 */
export async function logErrorAction(clockNumber: string, errorInfo: string): Promise<void> {
  const supabase = await createClient();
  ...
  try {
    ...
    if (updateError) {
      await recordHistoryAction(
        ...
        palletInfo.plt_num,
        palletInfo.plt_loc,
        `Update failed: ${getErrorMessage(updateError)}`
      );
      return {
        success: false,
        error: `Failed to update pallet: ${getErrorMessage(updateError)}`,
      };
    }
    ...
    try {
      ...
      return {
        success: false,
        error: `Failed to void pallet: ${getErrorMessage(voidError)}`,
      };
    }
    ...
    try {
      ...
      if (stockError) {
        ...
        await recordHistoryAction(
          ...
          palletInfo.plt_num,
          'Voided',
          `Stock level update failed: ${getErrorMessage(stockError)}`
        );
      } else {
        process.env.NODE_ENV !== 'production' &&
      ...
      await recordHistoryAction(
        ...
        palletInfo.plt_num,
        'Voided',
        `Stock level update error: ${getErrorMessage(stockUpdateError)}`
      );
    }
    ...
    await supabase.from('report_void').insert({
      plt_num: palletInfo.plt_num,
      reason: voidReason,
      damage_qty: 0, // Set to 0 for non-damage voids
      time: new Date().toISOString(),
    });
    ...
    await logErrorAction('unknown', `Void pallet error: ${getErrorMessage(error)}`);
    return {
      success: false,
  ...
  try {
    ...
    if (acoCheck.isACO && isPartialDamage) {
      return {
        success: false,
        error:
          'ACO Order Pallets do not support partial damage. If damaged, the entire pallet must be voided.',
      };
    }
    ...
    const hasTransactionService =
      inventoryService &&
      typeof inventoryService === 'object' &&
      'transactionService' in inventoryService;
    if (!hasTransactionService) {
      throw new Error('Transaction service not available');
    }
    // 策略4: unknown + type narrowing - 安全的服務訪問
    const serviceWithTransaction = inventoryService as unknown as {
      transactionService?: {
        executeTransaction: (
          callback: (client: unknown) => Promise<unknown>,
          options?: { description?: string; logTransaction?: boolean }
        ) => Promise<unknown>;
      };
    };

    if (!serviceWithTransaction.transactionService) {
      throw new Error('Transaction service not available');
    }

    try {
      // Execute damage operation within transaction
      const result = await serviceWithTransaction.transactionService.executeTransaction(
        async (client: unknown) => {
          // 策略4: unknown + type narrowing - 安全的 client 使用
      ...
      // 策略4: unknown + type narrowing - 安全處理 transaction 結果
      const transactionResult = result as { success?: boolean; error?: string };
      if (!transactionResult.success) {
        ...
        return {
          success: false,
          error: `Failed to update inventory: ${transactionResult.error || 'Unknown error'}`,
        };
      }
    } catch (error: unknown) {
      // Rollback pallet changes
    ...
    await logErrorAction('unknown', `Damage processing error: ${getErrorMessage(error)}`);
    return {
      success: false,
      error: `An unexpected error occurred: ${getErrorMessage(error)}`,
    };
...
function isMaterialGRNPallet(plt_remark: string | null): { isGRN: boolean; grnNumber?: string } {
  if (!plt_remark) {
    process.env.NODE_ENV !== 'production' && console.log('[GRN Check] No plt_remark found');
    return { isGRN: false };
  }
  ...
  process.env.NODE_ENV !== 'production' && console.log('[GRN Check] No GRN reference found');
  return { isGRN: false };
}
 ...
 */
async function deleteGRNRecord(pltNum: string): Promise<{ success: boolean; error?: string }> {
  const supabase = await createClient();
