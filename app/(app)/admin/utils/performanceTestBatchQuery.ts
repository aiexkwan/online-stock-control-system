...
class ConcurrentQueryPerformanceTester {
  ...
  async testConcurrentQuery(
    ...
    try {
      ...
      simplePerformanceMonitor.recordMetric(
        'concurrent_query_test_duration',
        result.duration,
        'performance'
      );
      simplePerformanceMonitor.recordMetric(
        'concurrent_query_test_requests',
        result.requestCount,
        'performance'
      );
      simplePerformanceMonitor.recordMetric(
        'concurrent_query_test_network_bytes',
        result.networkBytes || 0,
        'performance'
      );
  ...
  async testIndividualQueries(
    ...
    // 記錄到性能監控（使用簡化系統）
    simplePerformanceMonitor.recordMetric(
      'individual_queries_test_duration',
      result.duration,
      'performance'
    );
    simplePerformanceMonitor.recordMetric(
      'individual_queries_test_requests',
      result.requestCount,
      'performance'
    );
    simplePerformanceMonitor.recordMetric(
      'individual_queries_test_network_bytes',
      result.networkBytes || 0,
      'performance'
    );
  ...
  async comparePerformance(
    ...
    // 記錄比較結果（使用簡化系統）
    simplePerformanceMonitor.recordMetric(
      'performance_comparison_time_saved',
      timeSavedPercentage,
      'performance'
    );
    simplePerformanceMonitor.recordMetric(
      'performance_comparison_requests_reduced',
      requestsReducedPercentage,
      'performance'
    );
...
- **網絡傳輸減少**: ${this.formatBytes(
      (comparison.individualQueries.networkBytes || 0) -
        (comparison.concurrentQuery.networkBytes || 0)
    )}
...
`;
  ...
  getPerformanceStats() {
    ...
    const individualStats = simplePerformanceMonitor.getBasicStats(
      'individual_queries_test_duration'
    );
