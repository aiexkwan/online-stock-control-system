...
export function useDashboardConcurrentQuery(
  ...
  const [performanceMetrics, setPerformanceMetrics] =
    useState<UseDashboardConcurrentQueryReturn['performanceMetrics']>();
  ...
  const fetchConcurrentData = useCallback(async (): Promise<DashboardBatchQueryData> => {
    ...
    if (timeSinceLastFetch < MIN_FETCH_INTERVAL) {
      ...
      throw new Error(
        `Rate limited: Please wait ${Math.ceil((MIN_FETCH_INTERVAL - timeSinceLastFetch) / 1000)} seconds`
      );
    }
    ...
    simplePerformanceMonitor.recordMetric(
      'dashboard-concurrent_totalFetchTime',
      totalFetchTime,
      'performance'
    );
    simplePerformanceMonitor.recordMetric(
      'dashboard-concurrent_totalRequests',
      totalRequests,
      'performance'
    );
    simplePerformanceMonitor.recordMetric(
      'dashboard-concurrent_failedRequests',
      failedRequests,
      'performance'
    );
    simplePerformanceMonitor.recordMetric(
      'dashboard-concurrent_successRate',
      successRate,
      'performance'
    );
  ...
  // 重新獲取單個 widget 數據
  const refetchWidget = useCallback(
    async (widgetId: string) => {
      try {
        ...
        queryClient.setQueryData(queryKey, (oldData: DashboardBatchQueryData | undefined) => ({
          ...oldData,
          [widgetId]: widgetData,
        }));
