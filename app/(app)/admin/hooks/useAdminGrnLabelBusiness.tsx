    ...
    async (clockNumber: string) => {
      ...
      try {
        ...
        try {
          ...
          // Temporarily commented out to debug printing issue
          // if (abortSignal.aborted) {
          ...
          if (batchResult.success) {
            ...
            console.log('[DEBUG] RPC SUCCESS - About to process pallets');
            console.log('[DEBUG] RPC batchResult.data:', batchResult.data);
            ...
            console.log('[DEBUG] Extracted series:', series);
            console.log('[DEBUG] numberOfPalletsToProcess:', numberOfPalletsToProcess);
            ...
            if (
              ...
              logger.info('[DEBUG] Using unified PDF generation for GRN labels');

              // 準備 GRN Label 數據陣列
              const grnLabelDataArray: GrnLabelInputData[] = palletNumbers.map(
                (palletNumber, index) => ({
                  grnNumber: state.formData.grnNumber,
              ...
              const pdfResult = await generateBatch({
                ...
                dataArray: grnLabelDataArray,
                onProgress: (current, total, status, message) => {
                  // 更新進度狀態
                  const newStatus = Array(total)
                    .fill('Pending')
                    .map((_, idx) => {
                      if (idx < current) {
              ...
              // 收集生成的 PDF blobs
              collectedPdfBlobs = pdfResult.blobs;
              anyFailure = pdfResult.failed > 0;

              logger.info({
                msg: '[GRN PDF Generation]',
                successful: pdfResult.successful,
                failed: pdfResult.failed,
                totalBlobs: collectedPdfBlobs.length,
              });
            } else {
              console.log('[DEBUG] PALLET COUNT MISMATCH!');
              console.log('[DEBUG] Expected:', numberOfPalletsToProcess);
              console.log('[DEBUG] Received palletNumbers.length:', palletNumbers.length);
              console.log('[DEBUG] Received series.length:', series.length);
              console.log('[DEBUG] palletNumbers content:', palletNumbers);
              console.log('[DEBUG] series content:', series);

              logger.error({
                msg: 'Unified RPC pallet count mismatch',
            ...
            logger.error({
              msg: 'Unified RPC processing failed',
              error: new Error(batchResult.error || 'Unknown error'),
            });
          ...
          if (collectedPdfBlobs.length > 0) {
            ...
            if (anyFailure) {
              ...
              logger.info({
                msg: 'All labels generated successfully',
                count: collectedPdfBlobs.length,
              });
            ...
            if (collectedPdfBlobs.length > 0) {
              logger.debug({
                msg: '[GRN Print] Calling unified print service',
              ...
              try {
                ...
                await unifiedPrintService.printBatch(collectedPdfBlobs, {
                  productCode: state.productInfo.code,
                  palletNumbers: palletNumbers.slice(0, collectedPdfBlobs.length),
                  series: series.slice(0, collectedPdfBlobs.length),
                  grnNumber: state.formData.grnNumber,
                  supplierCode: state.supplierInfo.code,
                  operator: clockNumber,
                });

                logger.info('[GRN Print] PDFs sent to print queue');
              } catch (printError) {
                logger.error({
                  msg: '[GRN Print] Print error',
                  error: printError,
                });
                toast.error('Failed to send PDFs to print queue');
              }
            }
