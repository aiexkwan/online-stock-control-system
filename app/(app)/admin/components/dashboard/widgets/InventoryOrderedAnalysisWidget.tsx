...
export const InventoryOrderedAnalysisWidget: React.FC<InventoryOrderedAnalysisWidgetProps> = ({
    ...
    async (variables?: { productType?: string | null; productCodes?: string[] }) => {
      ...
      try {
        ...
        if (widgetData?.data && isWidgetApiDataWrapper(widgetData.data)) {
          const analysisResponse = InventoryAnalysisMapper.extractInventoryAnalysisFromWrapper(
            widgetData.data
          );
  ...
  if (finalLoading || !hasBeenInViewport) {
    return (
      <Card ref={containerRef} className={cn('h-full border-border bg-card/50')}>
      ...
      <Card ref={containerRef} className={cn('h-full border-border bg-card/50')}>
        <CardHeader className='pb-3'>
          <div className='flex items-center justify-between'>
            ...
            <span className='text-xs text-green-400'>✓ REST API</span>
          </div>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card ref={containerRef} className={cn('h-full border-border bg-card/50')}>
      <CardHeader className='pb-3'>
        <div className='flex items-center justify-between'>
          <CardTitle className={cn('flex items-center gap-2 text-lg', textClasses['heading-base'])}>
      ...
      <CardContent className='flex h-[calc(100%-4rem)] flex-col pt-0'>
        {/* 總體狀態卡片 */}
        <div className='mb-4'>
          <motion.div
          ...
          >
            ...
            <div className='mt-3 grid grid-cols-3 gap-3'>
              <div className='text-center'>
                ...
                <p className={cn(textClasses['heading-base'], 'font-bold text-foreground')}>
            ...
            <div className='mt-3 grid grid-cols-2 gap-3'>
              <div className='text-center'>
                ...
                <p
                  className={cn(textClasses['body-small'], 'font-bold')}
                  style={{ color: semanticColors.success.DEFAULT }}
                >
                  {summary.sufficientCount}
                </p>
              </div>
              <div className='text-center'>
                <p className={cn(textClasses['label-small'], 'text-muted-foreground')}>
          ...
          <div className='space-y-2'>
            {products.length > 0 ? (
                ...
                >
                  ...
                  <div className={cn('grid grid-cols-3 gap-2', textClasses['label-small'])}>
                    ...
                    <div>
                      ...
                      <span className='ml-1' style={{ color: semanticColors.warning.DEFAULT }}>
                        {product.orderDemand}
                      </span>
                    </div>
                    <div>
              ...
              <div className='flex h-full items-center justify-center'>
                <p className={cn(textClasses['body-base'], 'text-muted-foreground')}>
                  No products with active orders
                </p>
              </div>
            )}
          </div>
