...
import { BarChart3Icon, RefreshCwIcon, DatabaseIcon, AlertCircle } from 'lucide-react';
...
interface StockLevelWidgetConfig {
  params?: {
    warehouse?: string;
    dateRange?: {
      start?: string;
...
export default function StockLevelPOCWidget(props: WidgetComponentProps) {
  ...
  // Get config from widget or use defaults (memoized to prevent dependency changes)
  const config = useMemo(
    (): StockLevelWidgetConfig =>
      widget && 'config' in widget ? (widget.config as StockLevelWidgetConfig) : {},
    [widget]
  );
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const result = await unifiedDataLayer.getWidgetData<StockLevelData>('stock_levels', {
        warehouse: config.params?.warehouse,
        dateRange: timeFrame
          ? {
  ...
  useEffect(() => {
    fetchData();
  }, [timeFrame, widgetId, widget, config, fetchData]);
