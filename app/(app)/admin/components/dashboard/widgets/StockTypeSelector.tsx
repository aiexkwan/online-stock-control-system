...
import { StockData, StockChartMapper, isStockWidgetApiWrapper } from './types/StockChartTypes';
...
export const StockTypeSelector: React.FC<StockTypeSelectorProps> = ({ widget, isEditMode }) => {
        ...
        if (typesWidget && isStockWidgetApiWrapper(typesWidget.data)) {
          ...
          if (!errorMessage) {
            ...
            setProductTypes(
              types.length > 0
                ? types
                : ['EasyLiner', 'EcoPlus', 'Slate', 'SupaStack', 'Manhole', 'ACO']
            );
          } else {
  ...
  useEffect(() => {
    ...
    if (selectedType === 'all') {
      ...
      filtered = stockData.filter(item => item.type && item.type.toLowerCase() !== 'material');
    } else {
  ...
  if (isLoading) {
    return (
      <div className='flex h-full items-center justify-center'>
        <div className='h-2 w-16 rounded-full bg-gray-600 opacity-75' />
      </div>
    );
  }
      ...
      <div className='flex-1 overflow-auto'>
        {isLoadingStock ? (
          <div className='flex h-32 items-center justify-center'>
            <div className='h-2 w-12 rounded-full bg-gray-600 opacity-75' />
          </div>
        ) : (
        ...
        <div className='mt-4 flex justify-between border-t border-slate-700 pt-4 text-sm'>
          ...
          <span className='text-gray-300'>
            ...
            <span className='font-semibold text-white'>
              {filteredStockData
                .reduce((sum, item) => sum + (item.stock_level || 0), 0)
                .toLocaleString()}
            </span>
          </span>
        </div>
