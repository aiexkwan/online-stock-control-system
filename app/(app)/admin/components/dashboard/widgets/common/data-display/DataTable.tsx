...
import { Loader2, AlertCircle, RefreshCw, LucideIcon, ChevronDown } from 'lucide-react';
...
export function DataTable<T = Record<string, unknown>>({
  ...
  const getRowClassName = (item: T, index: number): string => {
    const baseClass =
      'rounded-lg bg-black/20 p-2 hover:bg-white/10 transition-colors cursor-pointer';
      ...
      <CardContent className='p-3'>
        ...
        {columns.length > 0 && (
          <div className={cn('mb-2 border-b border-slate-700 pb-2', headerClassName)}>
            <div
              className={`grid grid-cols-${columns.length} gap-2 px-2 text-xs font-medium text-slate-400`}
            >
              {columns.map(column => (
                <div
                  key={column.key}
