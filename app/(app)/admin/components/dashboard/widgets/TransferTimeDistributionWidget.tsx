...
export const TransferTimeDistributionWidget = React.memo(function TransferTimeDistributionWidget({
    ...
    try {
      ...
      if (
        ...
        const totalTransfers =
          TransferDistributionMapper.calculateTotalTransfers(processedTimeSlots);
  ...
  const data = useMemo<TransferTimeDistributionData>(() => {
    ...
    return {
      timeSlots: Array.isArray(timeSlots)
        ? TransferDistributionMapper.transformAPITimeSlots(timeSlots)
        : [],
  ...
  if (!isInViewport && !isEditMode) {
    return (
      <div ref={ref}>
        <LineChartSkeleton height='md' showHeader={true} />
      </div>
    );
  }
