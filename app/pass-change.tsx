...
export default function AlternatePasswordPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (userStr) {
        ...
        setUserData(user);
        console.log('替代密碼頁面：已載入用戶數據', user);
      } else {
        console.log('替代密碼頁面：未找到用戶數據');
      }
    } catch (e) {
      console.error('替代密碼頁面：解析用戶資訊失敗', e);
    }
  }, []);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-blue-50/30">
      <div className="bg-white p-8 rounded-lg shadow-lg max-w-md w-full">
        <h1 className="text-2xl font-bold mb-4">設置新密碼 (替代頁面)</h1>
