...
import { getAcoReportData, type AcoProductData } from '@/app/actions/reportActions';
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const productGroups = new Map<string, AcoProductData[]>();
    ...
    Array.from(productGroups.entries()).forEach(([productCode, items]) => {
      if (productIndex < 4) {
        ...
        // Fill pallet data - flatten all pallets from all items for this product
        const allPallets = items.flatMap(item => item.pallets);
        allPallets.forEach((pallet, palletIndex) => {
          if (currentRow + palletIndex <= 6 + maxRows) {
            const row = currentRow + palletIndex;
            sheet.getCell(row, baseCol + 1).value = pallet.plt_num || '';
            sheet.getCell(row, baseCol + 2).value = pallet.product_qty || 0;
            sheet.getCell(row, baseCol + 3).value = pallet.generate_time || '';
