  ...
  try {
    ...
    if (!reference) {
      return NextResponse.json(
        { error: 'ACO Order reference is required' },
        { status: 400 }
      );
    }
    ...
    mergeGroups.forEach((range) => {
      try {
        ...
        process.env.NODE_ENV !== "production" && process.env.NODE_ENV !== "production" && console.warn(`Could not merge cells for range ${range}:`, e);
      }
    });
    ...
    const formattedDate = today.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' }).toUpperCase();
