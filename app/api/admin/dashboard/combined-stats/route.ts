 ...
 */
export async function GET(request: Request): Promise<NextResponse<ApiResult<CombinedStats>>> {
  const startTime = Date.now();
  const requestId = Math.random().toString(36).substring(7);

  console.log(`[API-LOG-${requestId}] Combined stats API called at ${new Date().toISOString()}`);

  const result = await handleAsync(async (): Promise<CombinedStats> => {
