  ...
  try {
    ...
    return NextResponse.json({
      ...
      performance: {
        ...
        errorRate: redisMetrics.errors > 0 ? 
          ((redisMetrics.errors / redisMetrics.totalRequests) * 100).toFixed(2) + '%' : '0%',
      },
      ...
      // 緩存優化建議
      recommendations: generateOptimizationRecommendations(redisMetrics, redisStats),
 ...
 * 生成緩存優化建議
 */
function generateOptimizationRecommendations(metrics: any, stats: any): string[] {
  const recommendations = [];

  // 命中率建議
  if (metrics.hitRate < 70) {
    recommendations.push('Cache hit rate is below 70%. Consider increasing TTL or pre-warming more data.');
  ...
  if (metrics.avgResponseTime > 50) {
    recommendations.push('Average response time is high. Consider optimizing Redis configuration or network latency.');
  }

  // 錯誤率建議
  if (metrics.errors > 0) {
    recommendations.push(`${metrics.errors} cache errors detected. Check Redis connectivity and logs.`);
  }
  ...
  if (stats.connections > 50) {
    recommendations.push('High number of Redis connections. Consider connection pooling optimization.');
  }
