...
interface EmailServiceResult {
  ...
  message?: string;
  emailId?: string;
  recipients?: string[] | { to: string[]; cc: string[] };
}
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    try {
      ...
      apiLogger.error('[PDF Analysis] Text extraction failed', {
    ...
    try {
      ...
      apiLogger.error('[PDF Analysis] Failed to read prompt file', {
        error: getErrorMessage(promptError),
      });
      logApiResponse('POST', '/api/analyze-order-pdf-new', 500, Date.now() - startTime);
      return NextResponse.json(
        {
    ...
    try {
      ...
      try {
        ...
        apiLogger.error('[PDF Analysis] GPT-4-turbo also failed', {
      ...
      try {
        ...
        if (acoRecords.length > 0) {
          try {
            ...
            if (acoError) {
              apiLogger.error('[PDF Analysis] ACO insertion failed', {
            ...
            apiLogger.error('[PDF Analysis] ACO insertion error', {
              error: getErrorMessage(acoError),
            });
          }
        }
    ...
    return NextResponse.json(
      {
        ...
        errorCode: (error as ExtendedError).code,
