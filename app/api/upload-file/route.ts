...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    process.env.NODE_ENV !== 'production' && console.log('[Upload File API] 接收文件上傳請求...');
    ...
    if (error) {
      ...
      if (
        getErrorMessage(error).includes('row-level security') ||
        getErrorMessage(error).includes('RLS')
      ) {
        return NextResponse.json(
          {
