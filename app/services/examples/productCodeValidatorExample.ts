  ...
  try {
    ...
    multipleResult.enrichedOrders.forEach((order, index) => {
      console.log(`產品 ${index + 1}:`, {
        ...
        confidence: order.confidence_score ? `${(order.confidence_score * 100).toFixed(1)}%` : 'N/A',
      });
    });
...
export async function batchProcessingExample(): Promise<void> {
  try {
    ...
    // 模擬從訂單系統獲取的產品代碼列表
    const orderCodes = [
      'ABC123', 'XYZ789', 'DEF456', 'GHI789',
      'abc124', 'xyz790', 'invalid001', 'test999',
      '', null, undefined, 'GOOD001'
    ...
    console.log(`有效: ${result.summary.valid} (${((result.summary.valid / result.summary.total) * 100).toFixed(1)}%)`);
    console.log(`已修正: ${result.summary.corrected} (${((result.summary.corrected / result.summary.total) * 100).toFixed(1)}%)`);
    console.log(`無效: ${result.summary.invalid} (${((result.summary.invalid / result.summary.total) * 100).toFixed(1)}%)`);
    ...
    if (needsAttention.length > 0) {
      ...
      needsAttention.forEach(order => {
        if (!order.is_valid) {
          ...
          console.log(`⚠️  已修正: "${order.original_code}" → "${order.product_code}" (信心度: ${(order.confidence_score! * 100).toFixed(1)}%)`);
        }
      });
    }
    ...
    for (const testCode of testCodes) {
      ...
      if (similarCodes.length > 0) {
        ...
        similarCodes.forEach((code, index) => {
          ...
          console.log(`  ${index + 1}. ${code.code} - ${code.description} (相似度: ${(similarity * 100).toFixed(1)}%)`);
        });
      } else {
        console.log('未找到相似代碼');
      }
...
export async function performanceMonitoringExample(): Promise<void> {
  try {
    ...
    for (const size of batchSizes) {
      const testCodes = Array(size).fill(0).map((_, i) => `TEST${i.toString().padStart(3, '0')}`);
...
export async function orderProcessingScenarioExample(): Promise<void> {
  try {
    ...
    const orderItems: OrderItem[] = [
      { id: '001', productCode: 'ABC123', quantity: 10, unitPrice: 25.50 },
      { id: '002', productCode: 'xyz789', quantity: 5, unitPrice: 15.75 },
      { id: '003', productCode: 'INVALID001', quantity: 2, unitPrice: 30.00 },
      { id: '004', productCode: 'abc124', quantity: 8, unitPrice: 22.25 },
      { id: '005', productCode: '', quantity: 1, unitPrice: 10.00 },
    ];
    ...
    if (correctedItems.length > 0) {
      ...
      correctedItems.forEach(item => {
        console.log(`- 項目 ${item.id}: "${item.productCode}" → "${item.validatedProductCode}" (信心度: ${(item.confidenceScore! * 100).toFixed(1)}%)`);
      });
    }
    ...
    if (invalidItems.length > 0) {
      ...
      invalidItems.forEach(item => {
        console.log(`- 項目 ${item.id}: "${item.productCode}" - 總值: $${item.totalValue.toFixed(2)}`);
      });
    }
    ...
    console.log(`有效項目值: $${validOrderValue.toFixed(2)} (${((validOrderValue / totalOrderValue) * 100).toFixed(1)}%)`);
