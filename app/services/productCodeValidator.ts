...
class ProductCodeValidator {
  ...
  private constructor() {
    this.logger.info('ProductCodeValidator initialized with config:', {
  ...
  public async validateAndEnrichCodes(codes: string[]): Promise<ValidationResult> {
    ...
    try {
      ...
      if (processingTime > 100) {
        this.logger.warn('Batch processing exceeded target time', {
          targetMs: 100,
  ...
  public async refreshCache(): Promise<void> {
    ...
    try {
      ...
      // 性能警告
      if (loadTime > 5000) {
        this.logger.warn('Cache refresh took longer than expected', {
          targetMs: 5000,
          actualMs: loadTime,
        });
      }
      ...
      this.logger.error('Cache refresh failed', {
        error: error instanceof Error ? error.message : 'Unknown error',
        loadTimeMs: loadTime,
