...
export async function undoLoadPallet(
  ...
  try {
    ...
    process.env.NODE_ENV !== 'production' && console.log(`[undoLoadPallet] Calling RPC function`);
    ...
    if (!data || typeof data !== 'object') {
      return {
        success: false,
        message: 'Invalid response from database operation',
      };
    }

    const result = data as unknown as UndoLoadResult;

    if (result.success) {
      process.env.NODE_ENV !== 'production' &&
        console.log(`[undoLoadPallet] Successfully undone:`, result);
    }
...
export async function loadPalletToOrder(
  ...
  try {
    ...
    const result = data as unknown as LoadPalletResult;
