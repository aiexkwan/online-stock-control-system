...
export async function getGrnReportData(
  ...
  if (trimmedGrnRef === '' || trimmedMaterialCode === '') {
    console.error('getGrnReportData: grnRef and materialCode cannot be empty');
    return { success: false, error: 'grnRef and materialCode cannot be empty' };
  }
...
export async function generateGrnReportExcel(
  ...
  try {
    ...
    if (!materialCodesResult.success || !materialCodesResult.data?.length) {
      return { success: false, error: 'No materials found for this GRN' };
    }

    const materialCodes = materialCodesResult.data;
    const allReportsData: GrnReportPageData[] = [];

    // Collect data for all material codes
    for (const materialCode of materialCodes) {
      const grnResult = await getGrnReportData(grnRef, materialCode);
      if (grnResult.success && grnResult.data) {
        allReportsData.push(grnResult.data);
      }
    }

    if (allReportsData.length === 0) {
      return { success: false, error: 'Failed to retrieve any report data' };
    }
