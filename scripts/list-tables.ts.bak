    ...
    try {
      ...
      if (!error && data) {
        ...
        data.forEach((row: DatabaseRecord) => {
          // 策略4: unknown + type narrowing - 安全的字符串使用
          const tableName = typeof row.table_name === 'string' ? row.table_name : String(row.table_name || 'unknown');
          const isKnown = knownTables.includes(tableName);
          const icon = isKnown ? '✅' : '🆕';
