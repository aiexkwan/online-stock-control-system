...
async function generateReport(results, comparisons, duration) {
  const report = {
    ...
    summary: {
      ...
      criticalAlerts: comparisons.reduce((sum, c) => 
        sum + c.alerts.filter(a => a.level === 'critical').length, 0
      ),
      warnings: comparisons.reduce((sum, c) => 
        sum + c.alerts.filter(a => a.level === 'warning').length, 0
      ),
      improvements: comparisons.reduce((sum, c) => 
        sum + c.improvements.length, 0
      )
    },
    details: comparisons,
...
function generateRecommendations(comparisons) {
  ...
  for (const [metric, count] of Object.entries(metricCounts)) {
    if (count >= 2) { // Multiple URLs affected
  ...
  for (const comparison of report.details) {
    ...
    if (criticalAlerts.length > 0) {
      ...
      for (const alert of criticalAlerts) {
        console.log(`   - ${alert.metric}: ${alert.current}ms (baseline: ${alert.baseline}ms) ${alert.change}`);
      }
    }
  }
