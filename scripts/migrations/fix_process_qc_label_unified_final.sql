-- Final fix for process_qc_label_unified with correct column names
...
BEGIN
    ...
    IF NOT FOUND THEN
        INSERT INTO record_inventory (
            ...
            gen_random_uuid(),
            p_product_code,
            p_count,  -- 新建時 injection 數量等於 pallet 數量
            0,
            0,
            0,
            0,
    ...
    v_stats := jsonb_build_object(
        ...
        'records_created', jsonb_build_object(
            ...
            'history', p_count,
            'inventory', 1,
            'aco_detail', CASE WHEN p_aco_order_ref IS NOT NULL THEN p_count ELSE 0 END
        ),
...
-- Add comment
COMMENT ON FUNCTION process_qc_label_unified IS 'Unified QC label processing function with correct column names. ACO order quantity updates are handled separately via API to avoid duplicate updates.';
