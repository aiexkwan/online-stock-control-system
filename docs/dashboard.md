...
## 概述
Home 頁面（原 Dashboard Access 頁面）是用戶登入後的主要入口頁面，提供系統概覽和歷史記錄查看功能。經過重構後，頁面專注於展示 History Log，提供清晰的操作記錄追蹤。

## 文件位置
- **主要文件**: `app/home/page.tsx`
- **佈局文件**: `app/home/layout.tsx`
- **重定向**: `app/dashboard/page.tsx` (重定向到 /home)

## 功能特色

### 1. History Log 卡片
- **記錄顯示**: 顯示系統操作的歷史記錄
- **顯示高度**: 30rem (相比原來增加 25%)
- **數據欄位**: 
  - Pallet Number (棧板號碼)
  - Product Code (產品代碼)
...
- **響應式設計**: 適配各種螢幕尺寸
- **動畫效果**: 流暢的記錄載入動畫
- **更大顯示區域**: History Log 高度增加 25%，可顯示更多記錄
...
**日期**: 2025年5月
**主要變更**:
- **路徑更名**: `/dashboard/access` → `/home` (更易理解)
- **UI 改進**: History Log 高度增加 25% (24rem → 30rem)
- **導航更新**: 所有相關導航路徑已更新
- **重定向**: 舊路徑自動重定向到新路徑
