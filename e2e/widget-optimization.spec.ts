...
class WidgetTestHelper {
  ...
  async refreshWidget(widgetTestId: string) {
    await this.page
      .locator(`[data-testid="${widgetTestId}"] [data-testid="widget-refresh"]`)
      .click();
  }
  ...
  async checkChartRendered(widgetTestId: string): Promise<boolean> {
    const chartContainer = this.page.locator(
      `[data-testid="${widgetTestId}"] canvas, [data-testid="${widgetTestId}"] svg`
    );
...
test.describe('Widget Optimization E2E Tests', () => {
  ...
  test('Widget 交互功能測試', async ({ page }) => {
    ...
    const expandButton = page.locator(
      '[data-testid="inventory-summary"] [data-testid="widget-expand"]'
    );
    if (await expandButton.isVisible()) {
      await expandButton.click();
      await expect(
        page.locator('[data-testid="inventory-summary"][data-expanded="true"]')
      ).toBeVisible();

      // 再次點擊收縮
      await expandButton.click();
      await expect(
        page.locator('[data-testid="inventory-summary"][data-expanded="false"]')
      ).toBeVisible();
    }
  });
  ...
  test('SSR widgets 的正確渲染', async ({ page }) => {
    // 檢查 SSR 標記的 widgets
    const ssrWidgets = ['stats-overview', 'key-metrics', 'system-status'];
