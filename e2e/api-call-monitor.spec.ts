...
test.describe('API Call Monitoring', () => {
  ...
  test('should monitor API calls on analysis theme with minimal widgets', async ({ page, authenticatedPage }) => {
    ...
    // 4. 分析 API 調用
    const uniqueApiEndpoints = new Set(apiCalls.map(call => {
  ...
  test('should compare full vs minimal analysis theme', async ({ page, authenticatedPage }) => {
    ...
    console.log(`Reduction: ${fullCalls - minimalCalls} calls (${Math.round(((fullCalls - minimalCalls) / fullCalls) * 100)}%)`);
