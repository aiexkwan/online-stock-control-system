...
test.describe('Admin Analysis Page - Error Monitoring', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('console', (msg) => {
      ...
      if (msg.text().includes('Critical dependency') || msg.text().includes('the request of a dependency')) {
  ...
  test('should login and navigate to admin/analysis without originalFactory.call error', async ({ page }) => {
    ...
    const originalFactoryError = errors.find(error => 
    ...
    const criticalDependencyWarnings = consoleMessages.filter(msg => 
      ...
      msg.includes('the request of a dependency')
    );
    ...
    const hasMainContent = await page.locator('main, [role="main"], .dashboard, .admin-content').count() > 0;
    expect(hasMainContent, 'Admin analysis page should have main content').toBe(true);
    
    console.log('✅ Test completed successfully - admin/analysis page loaded without originalFactory.call error');
  });
