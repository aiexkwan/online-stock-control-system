...
test.describe('Admin Sub-pages - Comprehensive Widget Testing', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('pageerror', (error) => {
      ...
      if (errorMessage.includes('widget') || errorMessage.includes('Widget') ||
          errorMessage.includes('registry') || errorMessage.includes('originalFactory')) {
  ...
  test('should test all admin sub-pages and their widgets comprehensively', async ({ page }) => {
    ...
    for (const subPage of keyPages) {
      ...
      try {
        ...
        const registeredWidgets = newMessages.filter(msg => 
          msg.includes('Registered:') || msg.includes('registered successfully')
        );
        ...
        console.log(`✅ Page ${subPage} - Widgets: ${widgetCount}, Registered: ${registeredWidgets.length}`);
        
        // Check for widget-specific errors on this page
        const pageErrors = errors.slice(previousErrorCount);
        const pageWidgetErrors = pageErrors.filter(error => 
          error.includes('widget') || error.includes('Widget') ||
          error.includes('registry') || error.includes('originalFactory')
        );
    ...
    const registryMessages = consoleMessages.filter(msg => 
      msg.includes('widget registry') || 
      msg.includes('Widget Registry') ||
      msg.includes('useWidgetRegistry')
    );
    
    const adapterMessages = consoleMessages.filter(msg => 
      msg.includes('Adapter') && (msg.includes('Starting') || msg.includes('Completed'))
    );
    ...
    // Step 4: Assertions
    expect(testResults.successfulPages, 'Key admin pages should load successfully').toBe(keyPages.length);
  ...
  test('should specifically test widget loading and rendering', async ({ page }) => {
    ...
    const registryInit = consoleMessages.find(msg => 
      msg.includes('Initializing widget registry') || 
      msg.includes('useWidgetRegistry')
    );
    ...
    for (const type of adapterTypes) {
      const adapterLogs = consoleMessages.filter(msg => 
        msg.includes(`${type}WidgetAdapter`) && msg.includes('Completed')
      );
    ...
    const lazyLoadElements = await page.locator('[data-testid*="lazy"], [class*="lazy"], .loading, .skeleton').count();
  ...
  test.afterEach(async ({ page }) => {
    ...
    const report = {
      ...
      widgets: {
        registryMessages: consoleMessages.filter(msg => 
          msg.includes('registry') || msg.includes('Registry')
        ).length,
        adapterMessages: consoleMessages.filter(msg => 
          msg.includes('Adapter')
        ).length,
        registeredWidgets: consoleMessages.filter(msg => 
