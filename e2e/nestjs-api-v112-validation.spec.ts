...
test.describe('NestJS v1.1.2 REST API Validation', () => {
  ...
  test('Detailed Health Check with Connection Monitoring', async ({ request }) => {
    ...
    console.log(
      `✅ Detailed Health Check: ${data.healthScore}% health, ${data.responseTime}ms response time`
    );
  });
  ...
  test('Pallets CRUD Operations', async ({ request }) => {
    ...
    console.log(
      `✅ Pallets CRUD: Retrieved ${listData.data.length} pallets, individual lookup for ${pltNum} successful`
    );
  });
  ...
  test('API Performance and Stability', async ({ request }) => {
    ...
    console.log(
      `✅ Performance Test: ${numRequests} concurrent requests successful, response time: ${responseTime}ms`
    );
  });
