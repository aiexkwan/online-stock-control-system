...
import { test, expect, Page, Response as PlaywrightResponse } from '@playwright/test';
...
class APITestHelper {
  ...
  async monitorAPIRequests(): Promise<APIMetrics[]> {
    ...
    this.page.on('response', async response => {
      ...
      if (url.includes('/graphql')) {
        ...
        // Strategy 2: DTO/自定義 type interface - 明確定義 API 指標結構
   ...
   * 測量響應時間
   */
  // Strategy 3: 第三方庫類型整合 - 使用 Playwright 專用 Response 類型
  private async measureResponseTime(response: PlaywrightResponse): Promise<number> {
    try {
      // Strategy 4: unknown + type narrowing - 安全的 timing 屬性訸問
      const timing = (response as any).timing;
      if (timing && typeof timing === 'function') {
        const timingData = await timing();
        return timingData.responseEnd - timingData.requestStart;
      }
      return 0;
    } catch {
      return 0;
...
test.describe('API Switching A/B Test', () => {
  ...
  test('should use GraphQL when REST API is disabled', async ({ page }) => {
    ...
    console.log(
      `GraphQL requests: ${graphqlRequests.length}, REST requests: ${restRequests.length}`
    );
  ...
  test('should fallback from REST to GraphQL on error', async ({ page }) => {
    ...
    console.log(
      `Failed REST: ${failedRestRequests.length}, Fallback GraphQL: ${graphqlRequests.length}`
    );
...
test.describe('API Monitoring and Alerts', () => {
  test('should detect high error rates', async ({ page }) => {
    ...
    const errorElements = await page
      .locator('[data-testid*="error"], .error-message, .alert-error')
      .count();
