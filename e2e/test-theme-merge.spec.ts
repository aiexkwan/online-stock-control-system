...
test.describe('Theme Merge Verification', () => {
  test('Test new operations-monitoring theme', async ({ page }) => {
    ...
    const historyTree = page.locator('[data-testid="history-tree"], .history-tree, [class*="history"]');
    await expect(historyTree).toBeVisible({ timeout: 10000 });
    
    // Check for department selector
    const departmentSelector = page.locator('[data-testid="department-selector"], [class*="department"]');
    ...
    const injectionHistoryTree = page.locator('[data-testid="history-tree"], .history-tree, [class*="history"]');
    ...
    const pipelineHistoryTree = page.locator('[data-testid="history-tree"], .history-tree, [class*="history"]');
