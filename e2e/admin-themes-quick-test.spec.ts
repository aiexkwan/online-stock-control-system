...
test.describe('Admin Themes Quick Test', () => {
  test('should verify recordMetric fix across critical themes', async ({
    page,
    authenticatedPage,
  }) => {
    ...
    for (const theme of CRITICAL_THEMES) {
      ...
      try {
        ...
        // 檢查 widgets
        widgetCount = await page
          .locator('[data-testid*="widget"], [class*="widget"], [class*="card"], .grid > div')
          .count();
        ...
        console.log(
          `  ${success ? '✅' : '❌'} ${theme}: ${performanceErrors.length === 0 ? '無 performance 錯誤' : '有 performance 錯誤'}`
        );
        console.log(
          `     Widgets: ${widgetCount}, 其他錯誤: ${allErrors.length - performanceErrors.length}`
        );
      } catch (error) {
    ...
    results.forEach(result => {
      ...
      console.log(
        `${status} ${result.theme.padEnd(12)} | ${result.loadTime.toString().padStart(4)}ms | widgets: ${result.widgetCount.toString().padStart(2)} | perf: ${perfStatus}`
      );
    });
    ...
    expect(
      allPerfErrors,
      `🚨 發現 Performance Monitor 錯誤: ${allPerfErrors.join(', ')}`
    ).toHaveLength(0);
  ...
  test('should verify specific performance monitor functionality', async ({
    page,
    authenticatedPage,
  }) => {
    ...
    expect(
      performanceErrors,
      `Performance Monitor 仍有錯誤: ${performanceErrors.join(', ')}`
    ).toHaveLength(0);
