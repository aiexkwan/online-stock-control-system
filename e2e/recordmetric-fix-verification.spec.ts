...
test.describe('recordMetric Fix Verification', () => {
  test('should verify recordMetric errors are fixed on admin injection page', async ({ page }) => {
    ...
    expect(recordMetricErrors, `❌ Found recordMetric errors: ${recordMetricErrors.join(', ')}`).toHaveLength(0);
  ...
  test('should verify no performanceMonitor errors during widget loading', async ({ page }) => {
    ...
    page.on('console', msg => {
      if (msg.type() === 'error') {
        const text = msg.text();
        if (text.includes('performance') || text.includes('recordMetric') || text.includes('performanceMonitor')) {
    ...
    page.on('pageerror', error => {
      ...
      if (message.includes('performance') || message.includes('recordMetric') || message.includes('performanceMonitor')) {
    ...
    for (const theme of themes) {
      ...
      try {
        ...
        console.log(`⚠️  ${theme} theme had loading issues, but checking for performance errors anyway`);
      }
    }
    
    // 驗證沒有 performance monitor 錯誤
    expect(performanceErrors, `Performance monitor errors found: ${performanceErrors.join(', ')}`).toHaveLength(0);
