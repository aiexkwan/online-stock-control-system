  ...
  test.describe('ProductDistributionChartWidget API', () => {
    test('should retrieve product distribution data', async ({ request }) => {
      ...
      if (data.value.length > 0) {
        ...
        // 驗證百分比計算 - Strategy 4: unknown + type narrowing
        const total = data.value.reduce((sum: number, item: Record<string, unknown>) => {
          const itemValue = typeof item.value === 'number' ? item.value : 0;
          return sum + itemValue;
        }, 0);
        const firstItemValue = typeof firstItem.value === 'number' ? firstItem.value : 0;
        const firstItemPercentage =
          typeof firstItem.percentage === 'number' ? firstItem.percentage : 0;
        const calculatedPercentage = (firstItemValue / total) * 100;
        expect(Math.abs(firstItemPercentage - calculatedPercentage)).toBeLessThan(0.1);
      }
    ...
    test('should filter by warehouse', async ({ request }) => {
      ...
      data.transactions.forEach((transaction: any) => {
        const involvesWarehouse =
          transaction.fromLocation === warehouse || transaction.toLocation === warehouse;
