...
test.describe('Admin Themes Remaining Test', () => {
  test('should test all remaining admin themes comprehensively', async ({
    page,
    authenticatedPage,
  }) => {
    ...
    for (const theme of REMAINING_THEMES) {
      ...
      try {
        ...
        const hasMainContent =
          (await page.locator('main, .main-content, [role="main"], .container').count()) > 0;
        ...
        console.log(
          `  ${success ? '✅' : '❌'} ${theme}: ${success ? '成功' : '失敗'} (${loadTime}ms)`
        );
        console.log(
    ...
    expect(
      ...
      `❌ 成功的 themes 太少: ${successfulThemes.length}/5`
    ).toBeGreaterThanOrEqual(3);

    // 3. 所有 themes 都應該返回 200 狀態碼
    const badResponseCodes = results.filter(r => r.responseCode !== 200);
    expect(
      badResponseCodes.length,
      `❌ 有 themes 返回非 200 狀態碼: ${badResponseCodes.map(r => `${r.theme}:${r.responseCode}`).join(', ')}`
    ).toBe(0);
  ...
  test('should test theme-specific functionality deep dive', async ({
    page,
    authenticatedPage,
  }) => {
    console.log('\n🔬 深度測試各 theme 特有功能...');
    ...
    for (const theme of REMAINING_THEMES) {
      ...
      try {
        ...
        expect(
          performanceErrors,
          `${theme} 有 performance monitor 錯誤: ${performanceErrors.join(', ')}`
        ).toHaveLength(0);
      } catch (error) {
  ...
  try {
    switch (theme) {
      case 'upload':
        ...
        if ((await page.locator('[class*="upload"], [data-testid*="upload"]').count()) > 0)
          features.push('上傳組件');
        if ((await page.locator('button:has-text("Upload"), button:has-text("上傳")').count()) > 0)
          features.push('上傳按鈕');
        break;

      case 'update':
        // 檢查更新相關功能
        if ((await page.locator('button:has-text("Update"), button:has-text("更新")').count()) > 0)
          features.push('更新按鈕');
        if ((await page.locator('[class*="update"], [data-testid*="update"]').count()) > 0)
          features.push('更新組件');
      ...
      case 'stock-management':
        // 檢查庫存管理功能
        if ((await page.locator('[class*="stock"], [data-testid*="stock"]').count()) > 0)
          features.push('庫存組件');
      ...
      case 'system':
        // 檢查系統相關功能
        if ((await page.locator('[class*="config"], [class*="setting"]').count()) > 0)
          features.push('系統設置');
        if ((await page.locator('[class*="admin"], [class*="management"]').count()) > 0)
          features.push('管理功能');
        if (
          (await page.locator('button:has-text("Configure"), button:has-text("設置")').count()) > 0
        )
          features.push('配置按鈕');
        break;
      ...
      case 'analysis':
        ...
        if ((await page.locator('[class*="chart"], [class*="graph"]').count()) > 0)
          features.push('圖表組件');
        if ((await page.locator('[class*="analysis"], [class*="report"]').count()) > 0)
          features.push('分析報告');
        break;
    }
