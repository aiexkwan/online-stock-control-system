...
export default class A11yReporter implements Reporter {
  ...
  private generateHtmlReport(report: A11yTestReport): string {
    ...
    const avgScore = (
      report.detailedResults.reduce((sum, r) => sum + r.wcagScore, 0) /
...
<body>
    ...
    <div class="section">
        ...
        <div class="wcag-grid">
            ${Object.entries(report.wcagCompliance)
              .map(
                ([principle, data]) => `
                <div class="wcag-card">
  ...
  private printConsoleSummary(report: A11yTestReport) {
    ...
    Object.entries(report.wcagCompliance).forEach(([principle, data]) => {
      console.log(
        `${this.translatePrinciple(principle)}: ${data.passed} 通過, ${data.failed} 失敗`
      );
    });
