...
export class A11yTester {
  ...
  private calculateWCAGCompliance(results: any): A11yTestResult['wcagCompliance'] {
    ...
    const principleViolations = {
      perceivable: violations.filter((v: any) => 
        WCAG_RULES.PERCEIVABLE.includes(v.id)
      ).length,
      operable: violations.filter((v: any) => 
        WCAG_RULES.OPERABLE.includes(v.id)
      ).length,
      understandable: violations.filter((v: any) => 
        WCAG_RULES.UNDERSTANDABLE.includes(v.id)
      ).length,
      robust: violations.filter((v: any) => 
        WCAG_RULES.ROBUST.includes(v.id)
      ).length,
    };
    
    const totalViolations = Object.values(principleViolations).reduce((sum, count) => sum + count, 0);
...
export async function quickA11yCheck(page: Page): Promise<void> {
  ...
  // 斷言沒有嚴重的 A11y 問題
  expect(results.failedRules.filter(rule => 
    rule.impact === A11ySeverity.CRITICAL
  )).toHaveLength(0);
...
export class A11yReportGenerator {
  static generateReport(results: A11yTestResult[]): string {
    ...
    const averageScore = results.reduce((sum, r) => sum + r.wcagCompliance.overallScore, 0) / totalTests;
