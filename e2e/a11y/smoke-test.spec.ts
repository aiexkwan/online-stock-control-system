...
test.describe('🚀 A11y 煙霧測試 - 5分鐘快速驗證', () => {
  ...
  test.describe('📱 響應式設計 A11y 檢查', () => {
    ...
    test('手機版本 - 基本 A11y 檢查', async ({ page }) => {
      ...
      for (const button of buttons.slice(0, 5)) { // 只檢查前 5 個以節省時間
        if (await button.isVisible()) {
  ...
  test.describe('🔧 特殊功能 A11y 檢查', () => {
    ...
    test('跳轉連結 - 功能性檢查', async ({ page }) => {
      ...
      // 檢查跳轉連結
      const skipLinks = await page.locator('a[href*="#main"], a[href*="#content"], [class*="skip"]').all();
    ...
    test('5分鐘完整煙霧測試', async ({ page }) => {
      ...
      for (const { url, name } of CRITICAL_PAGES) {
        ...
        try {
          ...
          // 檢查是否有嚴重問題
          const criticalIssues = results.failedRules.filter(rule => 
      ...
      console.log(`🎯 煙霧測試完成: ${passedChecks}/${totalChecks} 通過 (${(passRate * 100).toFixed(1)}%)`);
