  ...
  test.describe('1.1 文本替代 (Text Alternatives)', () => {
    ...
    test('1.1.1 非文本內容 - 所有圖像都有適當的替代文本', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        const imageAltViolations = results.failedRules.filter(rule => 
          rule.id === 'image-alt' || rule.id === 'input-image-alt'
        );
  ...
  test.describe('1.2 時間為基礎之媒體 (Time-based Media)', () => {
    ...
    test('1.2.2 字幕 - 檢查自動播放設定', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        const results = await a11yTester.runSmokeTest();
        const autoplayViolations = results.failedRules.filter(rule => 
  ...
  test.describe('1.3 適應性 (Adaptable)', () => {
    ...
    test('1.3.1 資訊和關係 - 語義化標籤結構', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 使用 axe-core 進行更深入的檢查
        const results = await a11yTester.runSmokeTest();
        const structureViolations = results.failedRules.filter(rule => 
        ...
        for (const selector of CRITICAL_COMPONENTS) {
          ...
          for (const element of elements) {
            if (await element.isVisible()) {
              const results = await a11yTester.runSmokeTest();
              const contrastViolations = results.failedRules.filter(rule => 
                rule.id === 'color-contrast'
              );
              
              expect(
                contrastViolations,
                `對比度不足 in ${selector} at ${url}`
              ).toHaveLength(0);
            }
          }
        }
  ...
  test.describe('快速煙霧測試 (Smoke Test)', () => {
    ...
    test('5分鐘快速可感知性檢查', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 確保基本可感知性分數達到 85%
        expect(
          results.wcagCompliance.perceivable,
          `可感知性合規性不足 in ${url}`
        ).toBe(true);
      }
    });
  ...
  test.describe('回歸測試 (Regression Test)', () => {
    ...
    test('30分鐘全面可感知性檢查', async ({ page }) => {
      ...
      for (const url of TEST_URLS) {
        ...
        // 確保沒有中等以上的可感知性問題
        const significantViolations = perceivableResults.failedRules.filter(rule => 
