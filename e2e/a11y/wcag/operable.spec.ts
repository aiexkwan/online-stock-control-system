  ...
  test.describe('2.2 充足時間 (Enough Time)', () => {
    ...
    test('2.2.1 時間調整 - 檢查時間限制', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 檢查是否有計時器或時間限制
        const timerElements = await page.locator(
          '[class*="timer"], [class*="countdown"], [id*="timer"]'
        ).all();
        ...
        // 檢查會話超時
        const sessionWarnings = await page.locator(
          '[class*="session"], [class*="timeout"], [class*="expire"]'
        ).all();
        
        for (const warning of sessionWarnings) {
          if (await warning.isVisible()) {
            // 確保有延長會話的選項
            const extendOptions = await page.locator(
              'button[class*="extend"], button[class*="continue"]'
            ).count();
    ...
    test('2.2.2 暫停、停止、隱藏 - 檢查移動內容控制', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        const animatedElements = await page.locator(
          '[class*="animate"], [class*="spin"], [class*="pulse"]'
        ).all();
        
        for (const element of animatedElements) {
          if (await element.isVisible()) {
            // 檢查是否有暫停動畫的控制項
            const pauseControls = await page.locator(
  ...
  test.describe('2.3 癲癇和物理反應 (Seizures and Physical Reactions)', () => {
    ...
    test('2.3.1 三次閃光或低於臨界值 - 檢查閃光內容', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 檢查可能造成閃光的元素
        const flashingElements = await page.locator(
    ...
    test('2.4.4 連結目的（在內容中）- 檢查連結文字', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        for (const link of links) {
          ...
          const isVague = vagueLinkTexts.some(vague => 
            accessibleText && accessibleText.toLowerCase().includes(vague)
          );
          
          expect(
            !isVague,
            `連結文字過於模糊: "${accessibleText}" in ${url}`
          ).toBe(true);
        }
      }
    });
  ...
  test.describe('2.5 輸入方式 (Input Modalities)', () => {
    ...
    test('2.5.1 指標手勢 - 檢查複雜手勢', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 檢查需要複雜手勢的元素
        const gestureElements = await page.locator(
          '[class*="swipe"], [class*="pinch"], [class*="rotate"]'
        ).all();
        
        for (const element of gestureElements) {
          if (await element.isVisible()) {
            // 檢查是否有替代的單點操作
            const alternatives = await page.locator(
    ...
    test('2.5.2 指標取消 - 檢查點擊取消', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        const actionButtons = await page.locator(
          'button[class*="delete"], button[class*="remove"], button[class*="submit"]'
        ).all();
    ...
    test('2.5.3 名稱中的標籤 - 檢查可訪問名稱', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 檢查有可見標籤的控制項
        const labeledControls = await page.locator(
    ...
    test('2.5.4 動作啟動 - 檢查意外啟動', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 檢查可能意外啟動的元素
        const motionElements = await page.locator(
          '[class*="shake"], [class*="tilt"], [class*="motion"]'
        ).all();
        
        for (const element of motionElements) {
          if (await element.isVisible()) {
            // 檢查是否有關閉動作啟動的設定
            const disableOptions = await page.locator(
              'button[class*="disable"], [class*="settings"]'
            ).count();
  ...
  test.describe('快速煙霧測試 (Smoke Test)', () => {
    ...
    test('5分鐘快速可操作性檢查', async ({ page }) => {
      for (const url of TEST_URLS) {
        ...
        // 確保沒有嚴重的可操作性問題
        const criticalViolations = results.failedRules.filter(rule => 
        ...
        // 確保基本可操作性分數達到 85%
        expect(
          results.wcagCompliance.operable,
          `可操作性合規性不足 in ${url}`
        ).toBe(true);
      }
    });
  ...
  test.describe('回歸測試 (Regression Test)', () => {
    ...
    test('30分鐘全面可操作性檢查', async ({ page }) => {
      ...
      for (const url of TEST_URLS) {
        ...
        // 確保沒有中等以上的可操作性問題
        const significantViolations = operableResults.failedRules.filter(rule => 
          rule.impact === 'critical' || rule.impact === 'serious'
        );
      ...
      // 計算總體可操作性分數
      const averageScore = allResults.reduce((sum, r) => sum + r.wcagCompliance.overallScore, 0) / allResults.length;
