...
test.describe('Admin Themes Comprehensive Testing', () => {
  ...
  test('should test all admin themes systematically', async ({ page, authenticatedPage }) => {
    ...
    for (const theme of ADMIN_THEMES) {
      ...
      try {
        ...
        // 計算 widgets 數量
        try {
          const widgetElements = await page.locator('[data-testid*="widget"], [class*="widget"], [class*="card"]').count();
    ...
    results.forEach(result => {
      ...
      console.log(`${status} ${result.theme.padEnd(15)} | ${result.loadTime.toString().padStart(4)}ms | widgets: ${result.widgetCount.toString().padStart(2)} | perf: ${perfStatus}`);
    });
    ...
    expect(successfulThemes.length, `❌ 成功的 themes 太少: ${successfulThemes.length}/8`).toBeGreaterThanOrEqual(6);
    
    // 3. Critical themes (injection, pipeline, warehouse) 必須全部成功
    const criticalThemes = ['injection', 'pipeline', 'warehouse'];
    const successfulCriticalThemes = results.filter(r => 
      criticalThemes.includes(r.theme) && r.success
    );
    expect(successfulCriticalThemes.length, `❌ Critical themes 失敗過多`).toBe(criticalThemes.length);
  ...
  test('should test individual theme performance and stability', async ({ page, authenticatedPage }) => {
    // 對每個 theme 進行更深入的個別測試
    for (const theme of ADMIN_THEMES.slice(0, 3)) { // 只測試前 3 個 critical themes
      console.log(`\n🔬 深度測試 ${theme} theme...`);
      ...
      const perfErrors = errors.filter(error => 
        error.includes('recordMetric') || 
        error.includes('performanceMonitor')
      );
      
      expect(perfErrors, `${theme} theme 有 performance monitor 錯誤: ${perfErrors.join(', ')}`).toHaveLength(0);
