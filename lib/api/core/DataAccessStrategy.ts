...
export abstract class DataAccessLayer<TParams, TResult> {
  ...
  async fetch(
    params: TParams, 
    config: DataAccessConfig = { strategy: 'auto' }
  ): Promise<TResult> {
   ...
   */
  protected createCachedServerFetch(
    ttl: number = 60
  ): (params: TParams) => Promise<TResult> {
    return cache(
      async (params: TParams) => this.serverFetch(params),
      { revalidate: ttl }
    );
  }
}
...
export class DataAccessMetricsManager {
  ...
  getStats(operation: string): {
    ...
    const serverAvg = serverMetrics.length > 0
      ? serverMetrics.reduce((sum, m) => sum + m.duration, 0) / serverMetrics.length
      : 0;
      
    const clientAvg = clientMetrics.length > 0
