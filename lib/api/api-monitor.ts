...
export class APIMonitor {
  ...
  compareAPIs(timeRange?: { start: Date; end: Date }): APIComparison {
    ...
    const improvement = {
      responseTime: graphqlStats.avgResponseTime > 0 ? 
        ((graphqlStats.avgResponseTime - restStats.avgResponseTime) / graphqlStats.avgResponseTime) * 100 : 0,
      errorRate: graphqlStats.errorRate > 0 ? 
        ((graphqlStats.errorRate - restStats.errorRate) / graphqlStats.errorRate) * 100 : 0,
      successRate: graphqlStats.totalRequests > 0 ? 
        ((restStats.successfulRequests / restStats.totalRequests) - 
         (graphqlStats.successfulRequests / graphqlStats.totalRequests)) * 100 : 0,
    };
