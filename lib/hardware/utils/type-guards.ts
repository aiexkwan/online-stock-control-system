...
export function isQCLabelData(data: unknown): data is {
...
export function isGRNLabelData(data: unknown): data is {
  grn_number: string;
...
export function isReportData(data: unknown): data is {
  title: string;
  content: unknown[];
...
export function isDeviceConnectedData(data: unknown): data is {
  deviceId: string;
  deviceType: string;
  timestamp: string;
} {
...
export function isJobCompletedData(data: unknown): data is {
  jobId: string;
  success: boolean;
  duration?: number;
  error?: string;
...
export function isScanResultData(data: unknown): data is {
  scanData: string;
  format: string;
  confidence?: number;
} {
...
export function extractPrintJobData(job: PrintJob): {
  qcLabel?: ReturnType<typeof isQCLabelData> extends true ? Parameters<typeof isQCLabelData>[0] : never;
  grnLabel?: ReturnType<typeof isGRNLabelData> extends true ? Parameters<typeof isGRNLabelData>[0] : never;
  report?: ReturnType<typeof isReportData> extends true ? Parameters<typeof isReportData>[0] : never;
  raw: unknown;
} {
  const raw = job.data;
...
export function extractEventData(event: HardwareEvent): {
  deviceConnected?: ReturnType<typeof isDeviceConnectedData> extends true ? Parameters<typeof isDeviceConnectedData>[0] : never;
  jobCompleted?: ReturnType<typeof isJobCompletedData> extends true ? Parameters<typeof isJobCompletedData>[0] : never;
  scanResult?: ReturnType<typeof isScanResultData> extends true ? Parameters<typeof isScanResultData>[0] : never;
  raw: unknown;
} {
