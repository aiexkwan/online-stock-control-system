...
export function extractApiVersion(request: NextRequest): {
  ...
  if (API_VERSIONING_CONFIG.enableHeaderVersioning) {
    ...
    if (acceptVersion) {
      ...
      const normalizedVersion = acceptVersion.startsWith('v') 
        ? acceptVersion 
        : `v${acceptVersion}`;
...
export function validateApiVersion(version: string): {
  ...
  const versionInfo = API_VERSIONING_CONFIG.supportedVersions.find(
    v => v.version === version
  );
  ...
  if (!versionInfo) {
    return {
      ...
      error: `API version ${version} is not supported. Supported versions: ${
        API_VERSIONING_CONFIG.supportedVersions
          .map(v => v.version)
          .join(', ')
      }`,
    };
