...
export class PDFPerformanceMonitor extends EventEmitter {
  ...
  private updateResponseTimeMetrics(): void {
    ...
    // Average
    this.metrics.averageResponseTime = sorted.reduce((sum, time) => sum + time, 0) / len;
  ...
  private checkThresholds(metadata: RequestMetadata): void {
    ...
    if (metadata.responseTime > this.thresholds.maxResponseTime) {
      violations.push(
        `Response time ${metadata.responseTime}ms exceeds threshold ${this.thresholds.maxResponseTime}ms`
      );
    }

    if (metadata.tokensUsed > this.thresholds.maxTokensPerRequest) {
      violations.push(
        `Token usage ${metadata.tokensUsed} exceeds threshold ${this.thresholds.maxTokensPerRequest}`
      );
    }

    if (metadata.cost > this.thresholds.maxCostPerRequest) {
      violations.push(
        `Cost $${metadata.cost} exceeds threshold $${this.thresholds.maxCostPerRequest}`
      );
    }

    if (this.metrics.cacheHitRate < this.thresholds.minCacheHitRate) {
      violations.push(
        `Cache hit rate ${(this.metrics.cacheHitRate * 100).toFixed(1)}% below threshold ${this.thresholds.minCacheHitRate * 100}%`
      );
    }
  ...
  private collectSystemMetrics(): void {
    ...
    this.metrics.cpuUsagePercent = Math.min(
      100,
      Math.round((cpuUsage.user + cpuUsage.system) / 1000000 / 10)
    );
  ...
  public getPerformanceReport(): {
    ...
    if (this.metrics.averageResponseTime > 3000) {
      recommendations.push(
        'Average response time is high. Consider optimizing PDF processing or enabling parallel processing.'
      );
    }

    if (this.metrics.averageTokensPerRequest > 5000) {
      recommendations.push(
        'High token usage detected. Consider implementing text summarization or chunking strategies.'
      );
    }

    if (this.metrics.memoryUsageMB > 300) {
      recommendations.push(
        'High memory usage. Consider implementing memory cleanup or reducing cache size.'
      );
    }

    if (successRate < 0.95) {
      recommendations.push(
        'Success rate below 95%. Review error logs and implement retry mechanisms.'
      );
    }
