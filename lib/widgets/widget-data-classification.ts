...
export function getWidgetsByDataMode(mode: DataMode): WidgetDataClassification[] {
  return Object.values(widgetDataClassification).filter(widget => widget.mode === mode);
}
...
export function getWidgetsByDataStrategy(
  ...
  return Object.values(widgetDataClassification).filter(widget => widget.dataStrategy === strategy);
}
