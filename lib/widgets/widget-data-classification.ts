...
export interface WidgetDataClassification {
  widgetId: string;
  mode: DataMode;
  dataStrategy: 'graphql' | 'server-action' | 'mixed';
  notes?: string;
}
...
export const widgetDataClassification: Record<string, WidgetDataClassification> = {
  ...
  SupplierUpdateV2: {
    ...
    mode: 'read-write',
    dataStrategy: 'mixed',
    notes: '讀：GraphQL 搜索；寫：Server Actions。已遷移',
  },
  ReprintLabel: {
...
export function getWidgetsByDataStrategy(
  strategy: 'graphql' | 'server-action' | 'mixed'
...
export const dataClassificationStats = {
  ...
  byStrategy: {
    graphql: getWidgetsByDataStrategy('graphql').length,
