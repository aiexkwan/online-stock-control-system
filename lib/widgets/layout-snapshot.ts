...
import { AdminDashboardLayout, AdminWidgetConfig } from '@/app/admin/components/dashboard/adminDashboardLayouts';
import { WidgetLayoutItem } from './types';

// 將 grid area 字符串轉換為 x, y, w, h 座標
function parseGridArea(gridArea: string, gridTemplate: string): { x: number; y: number; w: number; h: number } {
  // 解析 grid template，先嘗試按引號分割（處理單行格式）
  const quotedSections = gridTemplate.match(/"[^"]+"/g) || [];
  
  // 如果有引號分割，每個引號內容代表一行
  const rows = quotedSections.length > 0 
    ? quotedSections.map(section => section.replace(/"/g, ''))
    : gridTemplate.trim().split('\n').filter(line => line.trim());
  ...
  if (!coords) {
    console.warn(`Grid area "${gridArea}" not found in template. Available areas:`, Array.from(gridMap.keys()));
...
export function validateLayoutSnapshot(
  ...
  for (let i = 0; i < original.widgets.length; i++) {
    ...
    const convertedWidget = converted.find(w => 
...
export function loadLayoutSnapshot(
  jsonData: string
): Record<string, WidgetLayoutItem[]> | null {
  try {
