...
export async function registerAnalysisWidgets(): Promise<void> {
  ...
  console.log(`[AnalysisWidgetAdapter] Completed: ${registeredCount} widgets registered in ${(endTime - startTime).toFixed(2)}ms`);
}
...
export async function preloadHighPriorityAnalysisWidgets(): Promise<void> {
  ...
  if (highPriorityWidgets.length > 0) {
    console.log('[AnalysisWidgetAdapter] Preloading high priority Analysis widgets:', highPriorityWidgets);
