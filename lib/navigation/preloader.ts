...
class LegacyUserBehaviorTracker {
  ...
  private getFrequencyBasedPredictions(frequency: Map<string, number>, currentPath: string): PredictedAction[] {
    const predictions: PredictedAction[] = [];
...
export class NavigationPreloader {
  ...
  async predictAndPreload(userId: string, currentPath: string): Promise<void> {
    // 計算在上一頁停留的時間
    const timeSpent = this.lastPath ? Math.floor((Date.now() - this.pathStartTime) / 1000) : undefined;
  ...
  getStats(): { cacheSize: number; queueLength: number; behaviorStats?: any } {
    return {
      ...
      behaviorStats: enhancedBehaviorTracker.getStats()
    };
  }
