...
export function useLoadingTimeout(options: UseLoadingTimeoutOptions): UseLoadingTimeoutResult {
  ...
  const handleTimeout = useCallback(async (): Promise<void> => {
    ...
    if (currentAttempt < retryCount) {
      ...
      retryTimeoutRef.current = setTimeout(async () => {
        ...
        await executeLoadingRef.current?.();
      }, delay);
    } else {
      // 達到最大重試次數，最終失敗
  ...
  // 執行載入邏輯的 ref，避免循環依賴
  const executeLoadingRef = useRef<() => Promise<void>>();

  // 執行載入邏輯
  const executeLoading = useCallback(async (): Promise<void> => {
    cleanupTimers();
    setIsTimedOut(false);
  ...
  // 更新 ref
  executeLoadingRef.current = executeLoading;
 ...
 * 簡化版超時載入 Hook
 */
export function useSimpleTimeout(id: string, timeout: number = 10000, retryCount: number = 2) {
 ...
 * API 超時載入 Hook
 */
export function useApiTimeout(id: string, timeout: number = 15000, retryCount: number = 3) {
  return useLoadingTimeout({
    id,
    type: 'api',
    timeout,
