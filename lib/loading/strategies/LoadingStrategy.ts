...
// 優先級調整係數
const PRIORITY_MODIFIERS: Record<LoadingPriority, {
...
export function createLoadingStrategy(
  ...
  // 應用性能調整
  let performanceModifier = { debounceMultiplier: 1, timeoutMultiplier: 1, minShowTimeMultiplier: 1, retryMultiplier: 1 };
  
  if (performanceMetrics && baseStrategy.performanceAware) {
    if (performanceMetrics.isLowEndDevice) {
      performanceModifier = combineModifiers(performanceModifier, PERFORMANCE_MODIFIERS.lowEndDevice);
    }
    
    if (performanceMetrics.isSlowNetwork) {
      performanceModifier = combineModifiers(performanceModifier, PERFORMANCE_MODIFIERS.slowNetwork);
    } else if (!performanceMetrics.isSlowNetwork && performanceMetrics.downlink > 5) {
      performanceModifier = combineModifiers(performanceModifier, PERFORMANCE_MODIFIERS.fastNetwork);
    }
  }
...
function combineModifiers(
  base: { debounceMultiplier: number; timeoutMultiplier: number; minShowTimeMultiplier: number; retryMultiplier: number },
  additional: { debounceMultiplier: number; timeoutMultiplier: number; minShowTimeMultiplier: number; retryMultiplier: number }
) {
  return {
 ...
 * 性能感知策略建議
 */
export function getPerformanceAwareStrategySuggestion(
  performanceMetrics: PerformanceMetrics
): {
  useSkeleton: boolean;
