...
export class PerformanceDetector {
  ...
  private getNetworkInfo() {
    ...
    if (connection) {
      return {
        // Strategy 4: unknown + type narrowing - 安全處理網絡類型
   ...
   * 判斷是否為低端設備
   */
  private isLowEndDevice(deviceInfo: {
    deviceMemory: number;
  ...
  public startNetworkMonitoring(callback: (metrics: PerformanceMetrics) => void): () => void {
    ...
    // Strategy 4: unknown + type narrowing - 安全事件監聽器
    if (connection && 'addEventListener' in connection) {
      (connection as EventTarget).addEventListener('change', handleChange);
    }

    return () => {
      if (connection && 'removeEventListener' in connection) {
        (connection as EventTarget).removeEventListener('change', handleChange);
      }
    };
  }
