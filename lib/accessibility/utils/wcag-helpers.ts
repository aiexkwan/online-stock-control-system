 ...
 * @returns 對比度比率和合規性資訊
 */
export function calculateColorContrast(foreground: string, background: string): ColorContrastRatio {
...
export function checkLargeTextContrast(
  ...
  const isLargeText =
    fontSize >= 24 ||
    (fontSize >= 18 &&
      (fontWeight === 'bold' || (typeof fontWeight === 'number' && fontWeight >= 700)));
...
export function getFocusableElements(container: HTMLElement): HTMLElement[] {
  ...
  return elements.filter(element => {
    return isElementFocusable(element) && isElementVisible(element);
  });
}
