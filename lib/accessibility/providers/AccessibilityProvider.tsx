...
export const AccessibilityProvider: React.FC<AccessibilityProviderProps> = ({
  ...
  const [keyboardConfig, setKeyboardConfig] =
    useState<KeyboardNavigationConfig>(defaultKeyboardConfig);
  ...
  useEffect(() => {
    ...
    if (preferences.focusOutlineAlwaysVisible) {
      root.style.setProperty(
        ...
        `${focusConfig.outlineWidth}px solid ${focusConfig.outlineColor}`
      );
    }
  ...
  const checkColorContrast = useCallback(
    (foreground: string, background: string): ColorContrastRatio => {
  ...
  const runAccessibilityAudit = useCallback(
    async (component: string): Promise<AccessibilityAuditResult> => {
