...
export function useKeyboardNavigation(
  ...
  // 合併配置
  const mergedConfig = { ...keyboardConfig, ...config };
 ...
 * @param options 配置選項
 */
export function useSimpleKeyboardNavigation(options: {
  containerRef?: React.RefObject<HTMLElement>;
  onEnter?: () => void;
  onEscape?: () => void;
  onSpace?: () => void;
...
export function useDirectionalNavigation(options: {
  ...
  const handleDirectionalKeyDown = useCallback((event: KeyboardEvent) => {
    const items = getItems();
    if (items.length === 0) return;
