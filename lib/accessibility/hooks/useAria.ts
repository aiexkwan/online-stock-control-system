...
export function useAria(defaultConfig?: Partial<AriaConfig>): UseAriaReturn {
  ...
  const getAriaProps = useCallback(
    (config: Partial<AriaConfig> = {}): Record<string, string | boolean | number | undefined> => {
   ...
   * @param expanded 是否展開
   */
  const updateAriaExpanded = useCallback(
    (elementId: string, expanded: boolean): void => {
   ...
   * @param selected 是否選中
   */
  const updateAriaSelected = useCallback(
    (elementId: string, selected: boolean): void => {
      const element = document.getElementById(elementId);
      if (element) {
   ...
   * @param element 元素
   */
  const ensureAriaCompliance = useCallback(
...
export function useAriaLabels(
  ...
  const getListItemLabel = useCallback(
    (item: string, index?: number, total?: number): string => {
      let label = item;
  ...
  const getButtonLabel = useCallback(
    (action: string, target?: string, state?: string): string => {
      let label = action;
  ...
  const getStatusLabel = useCallback(
    (element: string, status: string, value?: string | number): string => {
 ...
 */
export function useAriaLiveRegion(regionId?: string, priority: 'polite' | 'assertive' = 'polite') {
  ...
  const announce = useCallback(
    (message: string, customPriority?: 'polite' | 'assertive'): void => {
      const region = regionRef.current;
      if (!region) {
        // 回退到全域宣告
...
export function useAriaDescriptions(
  ...
  useEffect(() => {
    ...
    descriptions.forEach(({ id, content, persistent = true }) => {
      ...
      currentDescriptions.set(id, element);
    });
...
export function useAriaRole(defaultRole?: string) {
  ...
  const getRoleProps = useCallback(
    (role?: string, additionalProps?: Record<string, unknown>): Record<string, unknown> => {
