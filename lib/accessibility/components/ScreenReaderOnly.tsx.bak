    ...
    <Component
      className={cn(
        ...
        showOnFocus && [
          ...
          'focus-within:bg-black focus-within:p-2 focus-within:text-white',
          'focus-within:whitespace-nowrap focus-within:rounded focus-within:text-sm',
        ],

        // 調試模式：hover時顯示
        showOnHover && [
          'hover:not-sr-only hover:absolute hover:z-50',
          'hover:bg-black hover:p-2 hover:text-white',
          'hover:whitespace-nowrap hover:rounded hover:text-sm',
        ],
...
export const Status: React.FC<StatusProps> = ({ children, className, label }) => {
  return (
    <div className={cn('sr-only', className)} role='status' aria-label={label} aria-live='polite'>
      {children}
    </div>
  );
};
...
export const Alert: React.FC<AlertProps> = ({ children, className, label }) => {
  return (
    <div className={cn('sr-only', className)} role='alert' aria-label={label} aria-live='assertive'>
      {children}
    </div>
  );
};
...
export const Description: React.FC<DescriptionProps> = ({ children, id, className }) => {
  return (
...
export const Label: React.FC<LabelProps> = ({ children, id, className }) => {
  return (
    <div id={id} className={cn('sr-only', className)}>
      {children}
    </div>
...
export const Instructions: React.FC<InstructionsProps> = ({
  ...
  return (
    <div id={id} className={cn('sr-only', className)} aria-label={getAriaLabel()} role='note'>
      {children}
    </div>
  );
};
...
export const Progress: React.FC<ProgressProps> = ({ value, max = 100, description, className }) => {
...
export const Count: React.FC<CountProps> = ({ current, total, itemName = '項目', className }) => {
  return (
...
export function useLiveRegion() {
  ...
  const LiveRegionComponent = React.useCallback(
    () => <LiveRegion priority={priority}>{message}</LiveRegion>,
    [message, priority]
  );
  ...
  const WrappedComponent = React.forwardRef<HTMLElement, P>((props, ref) => {
    ...
    return (
      <>
        ...
        {/* 描述 */}
        {options.description && <ScreenReaderOnly>{options.description}</ScreenReaderOnly>}

        {/* 使用說明 */}
        {options.instructions && <Instructions type='usage'>{options.instructions}</Instructions>}
