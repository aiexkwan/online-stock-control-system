...
export const SkipLink: React.FC<SkipLinkProps> = ({
  ...
  return (
    <a
      ...
      className={cn(
        // 基本樣式
        'fixed z-[10001] px-4 py-2 font-medium text-white',
        'rounded-md border-2 border-blue-700 bg-blue-600',
        'whitespace-nowrap text-sm leading-none',

        // 隱藏和顯示邏輯
        'pointer-events-none opacity-0',
        '-translate-y-12 transform transition-all duration-200',
...
export const SkipLinks: React.FC<SkipLinksProps> = ({
  ...
  return (
    <nav
      className={cn('fixed z-[10001] flex flex-col gap-1', positionClasses[position], className)}
      aria-label='跳過連結導航'
    >
