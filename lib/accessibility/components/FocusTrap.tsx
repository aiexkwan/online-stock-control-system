...
export const FocusTrap = React.forwardRef<FocusTrapRef, FocusTrapProps>(
  (
    ...
    return (
      ...
      >
        {Component !== 'div' ? React.createElement(Component, { className }, children) : children}
      </div>
    );
  }
);
...
export const DialogFocusTrap: React.FC<DialogFocusTrapProps> = ({
  ...
  return (
    <FocusTrap {...focusTrapProps} active={open} className={className} as='div'>
      <div
        role='dialog'
        aria-modal='true'
...
export const MenuFocusTrap: React.FC<MenuFocusTrapProps> = ({
  ...
  return (
    ...
    >
      <div role={menuType} aria-label={label} tabIndex={-1}>
        {children}
      </div>
    </FocusTrap>
  );
...
export const FormFocusTrap: React.FC<FormFocusTrapProps> = ({
  ...
  return (
    <FocusTrap {...focusTrapProps} className={className} as='form'>
      <form onSubmit={handleSubmit} aria-label={label} noValidate>
        {children}
      </form>
    </FocusTrap>
  );
...
export function withFocusTrap<P extends object>(
  ...
  const WrappedComponent = React.forwardRef<HTMLElement, P & { focusTrapActive?: boolean }>(
    ({ focusTrapActive = true, ...props }, ref) => {
      return (
