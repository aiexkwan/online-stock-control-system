...
export class CardPerformanceMonitor {
  ...
  private recordMetric(metric: PerformanceMetric): void {
    ...
    if (metric.type === 'load_time' && metric.value > 500) {
      console.warn(`[Performance] Slow card load: ${metric.context?.cardId} (${metric.value.toFixed(2)}ms)`);
    }
  }
  ...
  generateReport(): PerformanceReport {
    ...
    const queryMetrics = {
      ...
      average: queryTimeMetrics.length > 0
        ? queryTimeMetrics.reduce((sum, m) => sum + m.value, 0) / queryTimeMetrics.length
        : 0,
      slowest: queryTimeMetrics.length > 0
        ? Math.max(...queryTimeMetrics.map(m => m.value))
        : 0
    };
