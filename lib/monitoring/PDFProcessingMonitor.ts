...
export class PDFProcessingMonitor {
  ...
  completeStage(fileId: string, stageName: string, error?: string): void {
    ...
    simplePerformanceMonitor.recordMetric(`pdf_stage_${stageName}`, stage.duration, 'pdf_stages');
  ...
  completeProcessing(fileId: string, success: boolean, error?: string): void {
    ...
    // Record completion
    simplePerformanceMonitor.recordMetric('pdf_processing_completed', success ? 1 : 0, 'pdf');

    if (metrics.totalDuration) {
      simplePerformanceMonitor.recordMetric('pdf_total_duration', metrics.totalDuration, 'pdf');
    }
  ...
  getStatistics(): {
    ...
    const durations = completed.filter(m => m.totalDuration).map(m => m.totalDuration!);
    const avgTotalDuration =
      durations.length > 0 ? durations.reduce((a, b) => a + b, 0) / durations.length : 0;

    // Average token reduction
    const tokenReductions = completed.filter(m => m.tokenUsage).map(m => m.tokenUsage!.reduction);
    const avgTokenReduction =
      tokenReductions.length > 0
  ...
  getRecommendations(): string[] {
    ...
    if (stats.avgTokenReduction < 30) {
      recommendations.push(
        'Token reduction below target (30%). Consider improving content optimization.'
      );
    }
    ...
    if (stats.cacheHitRate < 20) {
      recommendations.push(
        'Low cache hit rate. Consider extending cache TTL or warming cache with common patterns.'
      );
    }
    ...
    if (stats.avgTotalDuration > 30000) {
      recommendations.push(
        'Average processing time exceeds 30 seconds. Review OpenAI API performance.'
      );
    }
    ...
    if (stats.successRate < 95) {
      recommendations.push(
        'Success rate below 95%. Review error logs and implement better error recovery.'
      );
    }
    ...
    for (const [stage, stageStats] of Object.entries(stats.stageStats)) {
      if (
        stageStats &&
        stageStats.avg > 5000 &&
        stage !== PDFProcessingMonitor.STAGES.OPENAI_ANALYSIS
      ) {
        recommendations.push(
          `Stage "${stage}" taking ${Math.round(stageStats.avg)}ms on average. Consider optimization.`
        );
      }
    }
