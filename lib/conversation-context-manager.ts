...
export class ConversationContextManager {
  ...
  updateContext(question: string, sql: string, results: Record<string, unknown>[]) {
    ...
    // 添加到上下文（最新的在前）
    this.context.entities = [...newEntities, ...this.context.entities].slice(0, 50); // 只保留最近50個實體
  ...
  // 查找實體
  private findEntity(type: string, position: string): Entity | Entity[] | null {
    ...
    switch (position) {
      ...
      case 'recent':
        return candidates.slice(0, 5); // 返回最近5個
  ...
  // 從結果行提取實體
  private extractEntityFromRow(row: Record<string, unknown>): Entity {
    // 優先級：產品 > 訂單 > 棧板
    if (row.product_code) {
      return {
        type: 'product',
    ...
    return {
      type: 'product' as const, // 使用預設的產品類型
