...
export interface MovementResult {
  ...
  analysis?: {
    ...
    totalQuantity: number;
    mostCommonRoute?: { from: string; to: string; count: number; totalQuantity: number };
...
export class StockMovementService {
  ...
  async getMovementHistory(
    ...
    // Strategy 4: unknown + type narrowing - 簡化查詢避免 ParserError
  ...
  async analyzeMovementPatterns(
    ...
    if (error) {
      ...
      const totalQuantity = movements.reduce((sum, m) => {
      ...
      movements.forEach(m => {
        ...
        existing.totalQuantity += typeof m.quantity === 'number' ? m.quantity : 0;
      ...
      movements.forEach(m => {
        // Strategy 4: unknown + type narrowing - 安全日期轉換
        const dateValue = m.created_at || m.transfer_date;
        const hour = new Date(typeof dateValue === 'string' ? dateValue : new Date()).getHours();
    ...
    if ('quantity' in movement) {
      ...
      const quantity =
        ...
        typeof movement.quantity === 'number'
          ? movement.quantity
          : 0;
      // Strategy 4: unknown + type narrowing - 安全庫存檢查
      const inventoryValue =
        inventory && typeof inventory[fromLocation] === 'number' ? inventory[fromLocation] : 0;
      if (inventory && inventoryValue < quantity) {
        return {
          valid: false,
          reason: `Insufficient stock at ${fromLocation}. Available: ${inventoryValue}, Required: ${quantity}`,
          errors: [
            `Insufficient stock at ${fromLocation}. Available: ${inventoryValue}, Required: ${quantity}`,
          ],
        };
      }
    }
  ...
  private async checkInventory(
    ...
    // Strategy 4: unknown + type narrowing - 安全返回類型
   ...
   * 獲取移動驗證
   */
  private async validateLocationTransfer(
    fromLocation: string,
    toLocation: string
  ): Promise<boolean> {
