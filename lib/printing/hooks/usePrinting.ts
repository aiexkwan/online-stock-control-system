...
export function usePrinting(options: UsePrintingOptions = {}): UsePrintingReturn {
  ...
  // Print function
  const print = useCallback(async (request: PrintRequest): Promise<PrintResult> => {
    try {
      setPrinting(true);
      setProgress(0);
  ...
  const batchPrint = useCallback(async (batch: BatchPrintRequest): Promise<BatchPrintResult> => {
    try {
      setPrinting(true);
      setProgress(0);
  ...
  // Reprint function
  const reprint = useCallback(async (historyId: string): Promise<PrintResult> => {
    try {
      setPrinting(true);
      setProgress(0);
