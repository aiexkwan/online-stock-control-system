    ...
    const initializeMonitor = async () => {
      try {
        ...
        updateJobsHandler = async () => {
          try {
            // 策略4: unknown + type narrowing - 安全地獲取統計和模擬作業列表
            const monitorStats = await monitor.getStatistics();
            // monitorStats 來自 monitor.getStatistics() 返回的類型
            setStatistics({
              totalJobs: monitorStats.total,
              completedJobs: monitorStats.completed,
              failedJobs: monitorStats.failed,
              averageProcessingTime: 0, // HAL 不提供此數據，使用默認值
            });
