...
export class UnifiedPreloadService {
    ...
    const newTasks = tasks.filter(
      task =>
        !this.preloadQueue.some(
          existing => existing.type === task.type && existing.data.path === task.data.path
        )
    );
  ...
  private async processHighPriorityTasks() {
    ...
    await Promise.all(highPriorityTasks.map(task => this.executePreloadTask(task)));
  }
  ...
  private async processQueue() {
    ...
    try {
      ...
      // 並行執行
      await Promise.all(tasks.map(task => this.executePreloadTask(task)));
    } finally {
