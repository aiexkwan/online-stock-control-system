...
export class NotificationService {
  ...
  private initializeProviders(): void {
    this.providers.set(
      ...
      new EmailProvider() as unknown as NotificationProvider
    );
    this.providers.set(
      NotificationChannel.SLACK,
      new SlackProvider() as unknown as NotificationProvider
    );
    this.providers.set(
      NotificationChannel.WEBHOOK,
      new WebhookProvider() as unknown as NotificationProvider
    );
    this.providers.set(
      NotificationChannel.SMS,
      new SmsProvider() as unknown as NotificationProvider
    );
  }
  ...
  private async sendNotification(
    ...
    try {
      ...
      const result = await provider.send(
        alert,
        notification.config as unknown as Record<string, unknown>,
        content
      );
...
`;
  ...
  public async getNotificationStats(): Promise<NotificationStats> {
    try {
      ...
      const aggregatedStats = stats as unknown as {
        channel: string;
        status: string;
        count: number;
      }[];
      ...
      const notificationStats: NotificationStats = {
        ...
        sent: aggregatedStats
          .filter(item => item.status === 'sent')
          .reduce((sum, item) => sum + item.count, 0),
        failed: aggregatedStats
          .filter(item => item.status === 'failed')
          .reduce((sum, item) => sum + item.count, 0),
        pending: aggregatedStats
          .filter(item => item.status === 'pending')
          .reduce((sum, item) => sum + item.count, 0),
        byChannel: aggregatedStats.reduce(
          (acc, item) => {
            acc[item.channel as NotificationChannel] =
              (acc[item.channel as NotificationChannel] || 0) + item.count;
            return acc;
          },
          {} as Record<NotificationChannel, number>
        ),
        byStatus: aggregatedStats.reduce(
          (acc, item) => {
