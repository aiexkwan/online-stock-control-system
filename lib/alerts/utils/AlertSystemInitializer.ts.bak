...
export class AlertSystemInitializer {
  ...
  public async initialize(): Promise<AlertResponse> {
    try {
      ...
      if (!schemaResult.success) {
        throw new Error(
          `Database schema creation failed: ${(schemaResult as { message: string }).message}`
        );
      }
      ...
      if (!rulesResult.success) {
        throw new Error(
          `Default rules initialization failed: ${(rulesResult as { message: string }).message}`
        );
      }
      ...
      if (!templatesResult.success) {
        throw new Error(
          `Templates initialization failed: ${(templatesResult as { message: string }).message}`
        );
      }
      ...
      if (!monitoringResult.success) {
        throw new Error(
          `Monitoring service failed to start: ${(monitoringResult as { message: string }).message}`
        );
      }
  ...
  public async shutdown(): Promise<AlertResponse> {
    try {
      ...
      if (!result.success) {
        throw new Error(
          `Failed to stop monitoring service: ${(result as { message: string }).message}`
        );
      }
