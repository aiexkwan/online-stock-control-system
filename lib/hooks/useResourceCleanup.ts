...
export function useResourceCleanup(componentName?: string, debug = false) {
  ...
  const createInterval = useCallback(
    (callback: () => void, interval: number, name?: string): NodeJS.Timeout => {
      if (!isMountedRef.current) {
      ...
      debugLog('Interval created', {
        name,
        interval,
        totalActive: metricsRef.current.intervals.active,
      });
  ...
  const getMetrics = useCallback((): ResourceCleanupMetrics => {
    ...
    const totalActive =
      currentMetrics.timeouts.active +
