...
export function getUserFriendlyMessage(
  ...
  if (category === 'auth' || message.includes('auth') || message.includes('unauthorized')) {
    ...
    if (message.includes('unauthorized') || message.includes('401')) return ERROR_MESSAGES.AUTH.UNAUTHORIZED;
    if (message.includes('forbidden') || message.includes('403')) return ERROR_MESSAGES.AUTH.PERMISSION_DENIED;
