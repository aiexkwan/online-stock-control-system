...
export function ErrorFallback({
  ...
  return (
    <div
      className={`flex min-h-[200px] items-center justify-center rounded-lg border p-6 ${colors.bg} ${colors.border}`}
    >
      <div className='max-w-md space-y-4 text-center'>
        <div className='flex justify-center'>
          <div className={`rounded-full p-3 ${colors.bg}`}>{getSeverityIcon(severity)}</div>
        </div>
        ...
        <div>
          ...
          <p className='mb-4 text-sm text-gray-400'>{userMessage}</p>
          ...
          {process.env.NODE_ENV === 'development' && (
            <details className='mt-4 text-left'>
              ...
              <pre className='mt-2 max-h-32 overflow-auto rounded bg-gray-900/50 p-2 text-xs text-gray-400'>
                {error.stack}
              </pre>
            </details>
          )}
...
export function CompactErrorFallback({
  ...
  return (
    <div
      className={`flex items-center justify-between rounded-lg border p-3 ${colors[severity] || colors.medium} bg-gray-900/50`}
    >
      <div className='flex items-center gap-2'>
        <AlertTriangle className='h-4 w-4' />
        <span className='text-sm'>Error occurred</span>
      </div>
      <Button onClick={retry} size='sm' variant='outline' className='h-7 px-2'>
...
export function PageErrorFallback({ error, errorReport, retry, reset }: ErrorFallbackProps) {
  ...
  return (
    <div className='flex min-h-screen items-center justify-center bg-gray-900 p-6'>
      <Card className='w-full max-w-lg border-red-800/50 bg-red-950/20'>
        <CardHeader>
...
export function InlineErrorMessage({
  ...
  return (
    <div className={`flex items-center justify-between rounded-lg border p-3 ${getColors()}`}>
      ...
      {action && (
        <Button onClick={action.onClick} size='sm' variant='outline' className='h-7 px-3 text-xs'>
          {action.label}
        </Button>
      )}
    </div>
...
export function SuccessMessage({
  ...
  return (
    <div className='flex items-center justify-between rounded-lg border border-green-500/50 bg-green-950/20 p-3 text-green-400'>
      <div className='flex items-center gap-2'>
        <CheckCircle className='h-4 w-4' />
        <div>
          <span className='text-sm font-medium'>{message}</span>
          {description && <p className='mt-1 text-xs text-green-300'>{description}</p>}
        </div>
      </div>
      {action && (
        <Button
          ...
          size='sm'
          variant='outline'
          className='h-7 border-green-500/50 px-3 text-xs hover:bg-green-900/50'
        >
          {action.label}
        </Button>
      )}
