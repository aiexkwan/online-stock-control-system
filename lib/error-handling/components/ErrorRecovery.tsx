  ...
  useEffect(() => {
    if (canAutoRetry && !isRetrying) {
      const attemptRetry = async () => {
        try {
          await retry(onRecovery, autoRetry.maxAttempts, autoRetry.delayMs, autoRetry.backoffMultiplier);
        } catch (error) {
          onFailed();
        }
      };
        ...
        <div className="grid gap-2">
          {allActions.map((action, index) => (
            ...
            >
              ...
              {index === 0 && (
                <span className="ml-auto text-xs opacity-60">(Primary)</span>
              )}
            </Button>
          ))}
        </div>
...
export function RecoveryStatus({ 
  ...
  return (
    <div className="flex items-center gap-2 p-3 rounded-lg border border-blue-500/50 bg-blue-950/20">
      <div className="animate-spin">
        <RefreshCw className="w-4 h-4 text-blue-400" />
      </div>
...
export function RecoverySuccess({ 
  ...
  return (
    <div className="flex items-center gap-2 p-3 rounded-lg border border-green-500/50 bg-green-950/20">
