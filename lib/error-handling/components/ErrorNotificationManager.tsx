...
function BannerNotification({ 
  ...
  const severity = criticalErrors.length > 0 ? 'critical' : 
                  highErrors.length > 0 ? 'high' : 'medium';
  const config = NOTIFICATION_CONFIG[severity];
  const Icon = config.icon;
  
  return (
    <div className={`fixed top-0 left-0 right-0 z-50 ${config.className} border-b`}>
...
export function ErrorNotificationManager() {
  ...
  const handleSuccessNotification = React.useCallback((
    message: string, 
    description?: string
  ) => {
    toast.custom(
      () => <ToastSuccessContent message={message} description={description} />,
      {
        duration: 3000,
  ...
  const handleWarningNotification = React.useCallback((
    message: string, 
    description?: string
  ) => {
    toast.custom(
      () => (
        <div className="flex items-start gap-3 p-1">
          <AlertTriangle className="w-5 h-5 mt-1 flex-shrink-0 text-yellow-500" />
          <div className="flex-1 min-w-0">
            <p className="font-medium text-sm">{message}</p>
            {description && (
              <p className="text-xs opacity-80 mt-1">{description}</p>
  ...
  const handleInfoNotification = React.useCallback((
    message: string, 
    description?: string
  ) => {
    toast.custom(
