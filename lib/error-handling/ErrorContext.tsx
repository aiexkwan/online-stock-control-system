  ...
  switch (action.type) {
    case 'ADD_ERROR': {
      ...
      return {
        ...
        hasCriticalError: Array.from(newErrors.values()).some(error => error.severity === 'critical'),
    ...
    case 'RESOLVE_ERROR': {
      ...
      return {
        ...state,
        errors: newErrors,
        hasCriticalError: Array.from(newErrors.values()).some(error => error.severity === 'critical' && !error.resolved),
      };
    }
...
class ErrorUtils {
   ...
   */
  static generateUserMessage(error: Error, context: ErrorContextType, category: ErrorCategory): string {
    const message = error.message.toLowerCase();
...
export function ErrorProvider({ 
  ...
  const handleError = useCallback((
    ...
    const userMessage = options.userMessage || ErrorUtils.generateUserMessage(error, context, category);
    ...
    // Log to console
    console.error(`[ErrorProvider] ${severity.toUpperCase()} error in ${context.component}:`, error);
  ...
  const getComponentErrors = useCallback((component: string) => {
  ...
  const hasComponentErrors = useCallback((component: string) => {
