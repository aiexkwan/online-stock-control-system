...
export function useErrorHandler(componentName: string, defaultAction: string = 'unknown') {
  const { handleError, handleSuccess, handleWarning, handleInfo, clearComponentErrors } = useErrorContext();
  
  const createContext = useCallback((action: string = defaultAction, additionalData?: Record<string, any>): ErrorContext => ({
  ...
  const handleComponentError = useCallback((
    ...
    const context = createContext(action, options?.userMessage ? { customMessage: options.userMessage } : undefined);
...
export function useComponentErrorState(componentName: string) {
  ...
  const resolveComponentError = useCallback((errorId: string) => {
...
export function useGlobalErrorState() {
  ...
  const unresolvedErrors = Array.from(errorState.errors.values()).filter(
    err => !err.resolved
  );
