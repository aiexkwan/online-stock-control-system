...
export const EnhancedGlassmorphicCard: React.FC<EnhancedGlassmorphicCardProps> = ({
  ...
  const statusIndicator = useMemo(() => {
    ...
    return (
      <div className='absolute right-3 top-3 flex items-center gap-2'>
        <div className={`h-2 w-2 rounded-full ${statusConfig.dot}`} aria-hidden='true' />
  ...
  const loadingIndicator = useMemo(() => {
    ...
    return (
      <div className='absolute inset-0 flex items-center justify-center rounded-[inherit] bg-black/20 backdrop-blur-sm'>
  ...
  const dynamicStyles = useMemo((): CSSProperties => {
    const baseStyles: CSSProperties = {
      ...
      backdropFilter: perfConfig.useBackdropFilter
        ? `blur(${glassmorphicTheme.backdropBlur})`
        : 'none',
      WebkitBackdropFilter: perfConfig.useBackdropFilter
        ? `blur(${glassmorphicTheme.backdropBlur})`
        : 'none',
      border: 'none', // 移除所有邊框
  ...
  const animationProps =
    animate && perfConfig.enableAnimations
      ? {
