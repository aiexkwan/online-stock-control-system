...
export class StockHistoryDataLoader {
  ...
  static createPalletHistoryLoader(supabase: SupabaseClient) {
    return new DataLoader<
      string,
      { records: StockHistoryRecord[]; totalRecords: number; productCode: string }
    >(
        ...
        const results = await Promise.all(
          productCodes.map(async productCode => {
            try {
              ...
              console.log(
                `[DataLoader] Found ${palletNumbers.length} pallets for product ${productCode}`
              );
              ...
              const deduplicatedHistory = (historyData || []).filter(record => {
                ...
                if (seenRawRecords.has(uniqueKey)) {
                  console.warn(
                    `[DataLoader] Duplicate raw record detected and removed: ${uniqueKey}`
                  );
                  return false;
                }
              ...
              console.log(
                `[DataLoader] After deduplication: ${deduplicatedHistory.length} records`
              );
              ...
              deduplicatedHistory.forEach(record => {
                ...
                if (transformedRecords.has(finalUniqueKey)) {
                  console.warn(
                    `[DataLoader] Duplicate transformed record detected: ${finalUniqueKey}`
                  );
                  return;
                }
                ...
                transformedRecords.set(finalUniqueKey, {
                  ...
                  action:
                    record.action === 'Finished QC'
                      ? 'FINISHED_QC'
                      : record.action === 'GRN Receiving'
                        ? 'GRN_RECEIVING'
              ...
              console.log(
                `[DataLoader] Final deduplicated records for ${productCode}: ${records.length}`
              );
  ...
  static createSinglePalletHistoryLoader(supabase: SupabaseClient) {
    return new DataLoader<
      string,
      { records: StockHistoryRecord[]; palletNumber: string; totalRecords: number }
    >(
        ...
        const results = await Promise.all(
          palletNumbers.map(async palletNumber => {
            try {
              ...
              if (historyError) {
                console.error(
                  `Error fetching single pallet history for ${palletNumber}:`,
                  historyError
                );
              ...
              console.log(
                `[SinglePallet] Raw records for ${palletNumber}: ${historyData?.length || 0}`
              );
              ...
              const deduplicatedData = (historyData || []).filter(record => {
                ...
                if (seenRecords.has(uniqueKey)) {
                  console.warn(
                    `[SinglePallet] Duplicate record removed for ${palletNumber}: ${uniqueKey}`
                  );
                  return false;
                }
              ...
              if (deduplicatedData.length !== duplicateCount) {
                console.warn(
                  `[SinglePallet] Removed ${duplicateCount - deduplicatedData.length} duplicates for pallet ${palletNumber}`
                );
              }
              ...
              deduplicatedData.forEach(item => {
                ...
                transformedRecords.set(finalKey, {
                  ...
                  action:
                    item.action === 'Finished QC'
                      ? 'FINISHED_QC'
                      : item.action === 'GRN Receiving'
                        ? 'GRN_RECEIVING'
        ...
        const results = await Promise.all(
          dateRanges.map(async dateRange => {
            try {
              ...
              return {
                transfers:
                  data?.map((item: DatabaseTransferRecord) => ({
                    ...item,
                    ...
                    formattedDuration: item.duration ? `${Math.round(item.duration)}min` : null,
  ...
  static aggregateByOperator(records: StockHistoryRecord[]) {
    ...
    records.forEach(record => {
      ...
      operatorCounts.set(operator, {
        ...
        operatorId:
          typeof record.operatorId === 'number'
            ? record.operatorId
            : typeof record.operatorId === 'string'
              ? parseInt(record.operatorId, 10)
              : current.operatorId,
      });
    });
  ...
  static async getPalletHistoryByProduct(
    ...
    console.log(
      `[getPalletHistoryByProduct] Found ${palletNumbers.length} pallets for ${productCode}`
    );
    ...
    const sortField =
      fieldMapping[sort.field] ||
      fieldMapping[sort.field.toLowerCase()] ||
    ...
    if (filter.hasRemark !== undefined) {
      query = filter.hasRemark ? query.not('remark', 'is', null) : query.is('remark', null);
    }
    ...
    console.log(
      `[DEDUP] Final record count for ${productCode}: ${records.length} (removed ${(data?.length || 0) - records.length} duplicates)`
    );
    ...
    const productInfo = {
      ...
      activePallets:
        records.length > 0
          ? new Set(records.filter(r => r.action !== 'VOIDED').map(r => r.palletNumber)).size
          : 0,
  ...
  static async getPalletHistoryByNumber(
    ...
    try {
      ...
      if (!palletError && palletInfo) {
        ...
        console.warn(
          `[getPalletHistoryByNumber] No pallet info found for ${normalizedPalletNumber}:`,
          palletError
        );
      }
    ...
    console.log(
      `[getPalletHistoryByNumber] Deduplicated records for ${palletNumber}: ${records.length}/${data?.length || 0}`
    );
  ...
  static computePalletTimeline(records: StockHistoryRecord[]) {
    ...
    const movementRecords = sortedRecords.filter(r => ['TRANSFERRED', 'MOVED'].includes(r.action));
    ...
    return {
      ...
      totalDaysActive: Math.ceil(
        (lastMovement.getTime() - created.getTime()) / (1000 * 60 * 60 * 24)
      ),
      averageLocationStay:
        movementRecords.length > 1
          ? (lastMovement.getTime() - created.getTime()) /
            (1000 * 60 * 60 * 24) /
  ...
  static async computePalletCurrentStatus(
    ...
    return {
      ...
      daysInCurrentLocation: Math.ceil(
        (now.getTime() - lastActionAt.getTime()) / (1000 * 60 * 60 * 24)
      ),
    };
  }
      ...
      if (location && location !== currentLocation) {
        ...
        if (currentLocation && entryTime) {
          journey.push({
            ...
            duration: Math.round(
              (new Date(record.timestamp).getTime() - entryTime.getTime()) / (1000 * 60)
            ),
    ...
    switch (type) {
      case 'PRODUCT_CODE':
        ...
        if (productResults.data) {
          ...
          searchResults.push(
            ...productResults.data.map((item: DatabaseProductResult) => ({
              id: item.code,
      ...
      case 'PALLET_NUMBER':
        ...
        if (palletResults.data) {
          ...
          searchResults.push(
            ...palletResults.data.map((item: DatabasePalletResult) => ({
              id: item.plt_num,
  ...
  static async getStockHistoryStats(
    ...
    return {
      ...
      recentActivity: records.filter(
        (r: StatsRecord) => new Date(r.time).getTime() > now.getTime() - 24 * 60 * 60 * 1000
      ).length,
  ...
  static computeFlowMetrics(transfers: TransferData[]) {
    ...
    return {
      averageTransferDuration:
        durations.length > 0
          ? durations.reduce((sum: number, d) => sum + (d || 0), 0) / durations.length
          : 0,
      p95TransferDuration:
        durations.length > 0
          ? durations.sort((a, b) => (a || 0) - (b || 0))[Math.floor(durations.length * 0.95)]
          : 0,
  ...
  static analyzeOperatorPerformance(transfers: TransferData[]) {
    ...
    return Array.from(operatorMetrics.entries())
      .map(([operatorName, metrics]) => ({
        ...
        averageDuration:
          metrics.totalDuration / metrics.transfers.filter(t => t.duration).length || 0,
