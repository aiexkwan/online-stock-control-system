...
export class SchemaValidator {
  ...
  private validationCache = new Map<string, { result: SchemaValidationResult; timestamp: number }>();
  ...
  async validateAllSchemas(): Promise<SchemaValidationResult> {
    ...
    for (const [tableName, mapping] of Object.entries(DATABASE_FIELD_MAPPINGS)) {
      try {
        const validation = await FieldMapper.validateTableSchema(
          supabase, 
          tableName
        );
        ...
        valid = false;
        errors.push(`Failed to validate ${tableName}: ${error instanceof Error ? error.message : String(error)}`);
      }
    }
  ...
  async validateTableSchema(tableName: string): Promise<SchemaValidationResult> {
    ...
    try {
      ...
      const result: SchemaValidationResult = {
        ...
        errors: validation.valid ? [] : [`Missing columns in ${tableName}: ${validation.missing.join(', ')}`],
      ...
      const result: SchemaValidationResult = {
        valid: false,
        errors: [`Failed to validate ${tableName}: ${error instanceof Error ? error.message : String(error)}`],
        warnings: [],
        missingColumns: {}
      };
  ...
  static createValidationMiddleware() {
    ...
    return async (
      resolve: (root: unknown, args: unknown, context: unknown, info: GraphQLResolverInfo) => Promise<unknown>,
      root: unknown,
      args: unknown,
      context: unknown,
 ...
 */
export function withSchemaValidation<TRoot = unknown, TArgs = unknown, TContext = unknown, TResult = unknown>(
  resolver: (root: TRoot, args: TArgs, context: TContext, info: { fieldName: string }) => Promise<TResult>
) {
