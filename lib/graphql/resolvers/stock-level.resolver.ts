  ...
  Query: {
    stockLevelList: async (_parent, args, context: GraphQLContext) => {
      ...
      try {
        ...
        // We need to get the most recent update_time for each stock
        ...
        if (stockLevels) {
          stockLevels.forEach(record => {
            if (!latestByStock.has(record.stock) || 
        ...
        const records = Array.from(latestByStock.values()).map(record => ({
          uuid: record.uuid,
          stock: record.stock,
        ...
        throw error instanceof GraphQLError ? error : 
          new GraphQLError(`Failed to fetch stock levels: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    },
    ...
    stockLevelChart: async (_parent, args, context: GraphQLContext) => {
      ...
      try {
        ...
        throw error instanceof GraphQLError ? error : 
          new GraphQLError(`Failed to fetch stock level chart data: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    },
    ...
    stockLevelStats: async (_parent, args, context: GraphQLContext) => {
      ...
      try {
        ...
        throw error instanceof GraphQLError ? error : 
          new GraphQLError(`Failed to fetch stock level statistics: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    },
  },
};