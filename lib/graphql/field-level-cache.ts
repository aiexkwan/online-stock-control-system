...
export const FieldCacheConfigs: Record<string, FieldCacheConfig> = {
  ...
  'Pallet.movements': {
    ...
    keyGenerator: (parent, args) =>
      `pallet:${parent.palletNumber}:movements:${JSON.stringify(args)}`,
  },
  ...
  'Pallet.inventoryRecords': {
    ...
    maxSize: 1200,
    keyGenerator: (parent, args) =>
      `pallet:${parent.palletNumber}:inventory:${JSON.stringify(args)}`,
  },
  ...
  'Query.getPendingOrders': {
    ...
    maxSize: 100,
    keyGenerator: (parent, args) =>
      `pendingorders:${args.status}:${JSON.stringify(args.pagination)}`,
  },
  ...
  'Query.getActiveTransfers': {
    ...
    maxSize: 80,
    keyGenerator: (parent, args) =>
      `activetransfers:${JSON.stringify(args.dateRange)}:${JSON.stringify(args.pagination)}`,
  },
};
...
export class SmartCacheInvalidator {
  // 基於數據變更的智能失效
  static async invalidateOnDataChange(
    changeType: string,
    entityId: string,
    entityType: string
  ): Promise<void> {
  ...
  private static async invalidateInventoryRelated(productCode: string): Promise<void> {
    const fieldsToInvalidate = ['Product.inventory', 'Query.getLowStockProducts'];
  ...
  private static async invalidateOrderRelated(orderRef: string): Promise<void> {
    const fieldsToInvalidate = ['Query.getPendingOrders'];
  ...
  static async scheduledInvalidation(): Promise<void> {
    ...
    setInterval(
      async () => {
        // 每30分鐘清理低頻數據
        const lowFrequencyFields = ['Pallet.grnRecords', 'Query.getLowStockProducts'];
...
export class CacheMonitor {
  static generateCacheReport(): {
    ...
    const poorPerformingFields = performanceAnalysis.filter(
      f => f.performance === 'poor' && f.totalRequests > 10
    );
    if (poorPerformingFields.length > 0) {
      recommendations.push(
        `Consider adjusting TTL or caching strategy for: ${poorPerformingFields.map(f => f.field).join(', ')}`
      );
    }
    ...
    if (highVolumeFields.length > 0) {
      recommendations.push(
        `High volume fields may need cache size optimization: ${highVolumeFields.map(f => f.field).join(', ')}`
      );
    }
  ...
  static logCacheMetrics(): void {
    ...
    report.fieldDetails.forEach((field: any) => {
      console.log(
        `  ${field.field}: ${(field.hitRate * 100).toFixed(2)}% (${field.totalRequests} requests) - ${field.performance}`
      );
    });
