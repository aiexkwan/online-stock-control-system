...
(async () => {
  ...
  try {
    ...
    try {
      ...
      await page.type(
        'input[type="email"]',
        process.env.SYS_LOGIN || process.env.PUPPETEER_LOGIN || 'admin@test.com'
      );
      await page.type(
        'input[type="password"]',
        process.env.SYS_PASSWORD || process.env.PUPPETEER_PASSWORD || 'password'
      );
    ...
    const gridInfo = await page.evaluate(() => {
      const gridContainer =
        document.querySelector('.operations-grid-container') ||
    ...
    const cards = await page.evaluate(() => {
      ...
      const deptSelector =
        document.querySelector('[style*="dept-sel"]') ||
      ...
      // 檢查History Tree
      const historyTree =
        document.querySelector('[style*="hist"]') ||
